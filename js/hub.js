...
async function getPortfolioGeometry(p){
 ...
 let g=null;
 if(currentUser)g=await loadProjectGeometry(p.id);
 if(!g){
  const x=inferGeo(p);
  g={type:"point",source:"inferred",referencePoints:[{lat:x.lat,lon:x.lon}]};
 }
...
async function refreshPortfolioMapLayers(){
 ...
 for(const p of visible){
  ...
  if(g?.type==="segment"&&g.coordinates?.length>1)L.polyline(g.coordinates.map(c=>[c[1],c[0]]),{weight:4,opacity:.75}).bindPopup(`<b>${traceEscape(p.id)}</b><br>${traceEscape(p.name)}`).addTo(portfolioMapLayer);
  else{const q=g?.referencePoints?.[0];if(q)L.circleMarker([q.lat,q.lon],{radius:5,weight:2,fillOpacity:.7}).bindPopup(`<b>${traceEscape(p.id)}</b><br>${traceEscape(p.name)}`).addTo(portfolioMapLayer);}
 }
}
...
async function initPortfolioMap(){
 ...
 try{
  ...
  const batch=projects.filter(p=>!p.rejected);
  await Promise.all(batch.map(async p=>{
   const g=await getPortfolioGeometry(p);
   if(g?.type==="segment"&&g.coordinates?.length>1){
    L.polyline(g.coordinates.map(c=>[c[1],c[0]]),{weight:4,opacity:.75}).bindPopup(`<b>${traceEscape(p.id)}</b><br>${traceEscape(p.name)}`).addTo(portfolioMapLayer);
   }else{
    const q=g?.referencePoints?.[0];if(q)L.circleMarker([q.lat,q.lon],{radius:5,weight:2,fillOpacity:.7}).bindPopup(`<b>${traceEscape(p.id)}</b><br>${traceEscape(p.name)}`).addTo(portfolioMapLayer);
   }
  }));
  const layers=portfolioMapLayer.getLayers();if(layers.length){try{portfolioMap.fitBounds(L.featureGroup(layers).getBounds().pad(.05),{maxZoom:10})}catch(e){}}
...
(function injectProjectListStyles(){
  ...
  s.textContent=`
    ...
    .portfolio-map-section{margin:22px 0;padding:20px;border:1px solid var(--line);border-radius:18px;background:#fff}

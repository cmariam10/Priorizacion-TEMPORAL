...
function retryPortfolioMap(){
 ...
 const el=document.getElementById("portfolioProjectsMap");
 if(el)el.innerHTML="";
 setTimeout(initPortfolioMap,100);
}
...
(function injectProjectListStyles(){
  ...
  s.textContent=`
    ...
    .portfolio-map-retry{height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;text-align:center;color:var(--muted)}
...
function ensurePortfolioMapSection(){
 ...
 setTimeout(initPortfolioMap,150);
}

function renderList(){
  ensurePortfolioMapSection();
  const list=getGlobalFilteredProjects();
  if(portfolioMapReady&&portfolioMap)refreshPortfolioMapLayers();
  if(!projects.length){

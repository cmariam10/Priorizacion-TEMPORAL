...
window.HUB_ONEDRIVE_BUILD = "2026-09-21-downloadurl-from-list-v7";
...
async function openOneDriveDocument(d){
 ...
 try{
  ...
  headers.set("X-HUB-User",currentUser?.username||"");
  headers.set("X-HUB-Role",currentRole||"");
  if(!d.downloadUrl)throw new Error("El listado no entregó la URL temporal del documento. Cierre y vuelva a abrir Documentos.");
  const url=`${HUB_API.baseUrl}/projects/${encodeURIComponent(d.project)}/documents/${encodeURIComponent(d.itemId)}/content?downloadUrl=${encodeURIComponent(d.downloadUrl)}`;

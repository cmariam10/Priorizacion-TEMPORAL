...
// ===== Gobernanza central V24: ciclo de vida, vigencia y asignaciones =====
const GOVERNANCE_DEFAULTS={maxBusinessDays:180,warningBusinessDays:30,assignmentBusinessDays:10};
...
function deriveFreshness(p){const last=p.governance?.lastValidatedAt||excelUpdatedAt(p)||p.sharedUpdatedAt||'';const days=businessDaysBetween(last,new Date());if(days===null)return {code:'unknown',label:'Sin fecha de actualización',days:null,last};const max=Number(p.governance?.maxBusinessDays||GOVERNANCE_DEFAULTS.maxBusinessDays),warn=Number(p.governance?.warningBusinessDays||GOVERNANCE_DEFAULTS.warningBusinessDays);if(days>max)return {code:'update_required',label:'Requiere actualización',days,last};if(days>=Math.max(0,max-warn))return {code:'warning',label:'Próximo a actualizar',days,last};return {code:'current',label:'Vigente',days,last};}
function governanceBadge(p){const f=deriveFreshness(p);const icon=f.code==='update_required'?'🔴':f.code==='warning'?'🟡':f.code==='current'?'🟢':'⚪';return `${icon} ${f.label}`;}
function applyGovernance(p,g){p.governance=g||{};if(g?.lifecycleState)p.lifecycleState=g.lifecycleState;if(g?.assignment)p.assignment=g.assignment;}
async function loadAllGovernance(){try{const r=await hubApiFetch('/governance'),m=new Map();for(const g of r?.governance||[])if(g?.projectId)m.set(String(g.projectId),g);projects.forEach(p=>applyGovernance(p,m.get(String(p.id))||null));return m;}catch(e){console.warn('No se pudo sincronizar gobernanza',e);return new Map();}}
async function saveProjectGovernance(patch,id){const p=projects.find(x=>x.id===id);if(!p)throw new Error('Proyecto no encontrado.');const body={projectId:id,lifecycleState:p.lifecycleState||p.governance?.lifecycleState||'',lastValidatedAt:p.governance?.lastValidatedAt||excelUpdatedAt(p)||new Date().toISOString(),maxBusinessDays:Number(p.governance?.maxBusinessDays||GOVERNANCE_DEFAULTS.maxBusinessDays),warningBusinessDays:Number(p.governance?.warningBusinessDays||GOVERNANCE_DEFAULTS.warningBusinessDays),assignment:p.assignment||p.governance?.assignment||null,...patch,updatedBy:actorLabel()};const r=await hubApiFetch(`/projects/${encodeURIComponent(id)}/governance`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});applyGovernance(p,r?.governance||body);return r?.governance||body;}
async function confirmProjectUpdate(id){try{await saveProjectGovernance({lastValidatedAt:new Date().toISOString()},id);await loadProjectsFromExcel();alert(`${id} quedó marcado como actualizado y vigente.`);}catch(e){alert('No fue posible actualizar la vigencia: '+e.message);}}
async function markProjectExecuted(id){const p=projects.find(x=>x.id===id);if(!p)return;const note=prompt('Observación de finalización de la ejecución (opcional):','')??null;if(note===null)return;try{await saveProjectGovernance({lifecycleState:'Ejecutado',executedAt:new Date().toISOString(),executionNote:note,assignment:null},id);p.state='Ejecutado';p.lifecycleState='Ejecutado';p.assignment=null;renderKpis();renderList();renderInbox();await renderNotifications();openManageModal(id,true);alert(`${id} quedó marcado como Ejecutado. El registro es central y visible para todos los usuarios.`);}catch(e){alert('No fue posible marcar el proyecto como ejecutado: '+e.message);}}
async function assignCurrentProjectWork(id){const p=projects.find(x=>x.id===id);if(!p)return;const activity=steps.find(s=>s.n===Number(p.step))?.title||'Gestión del proyecto';const role=steps.find(s=>s.n===Number(p.step))?.role||p.owner||'';const days=Number(prompt('Plazo de la asignación en días hábiles:',String(GOVERNANCE_DEFAULTS.assignmentBusinessDays)));if(!Number.isFinite(days)||days<1)return;const now=new Date().toISOString();const assignment={activity,responsibleRole:role,responsible:p.owner||role,institution:p.inst||'',assignedAt:now,businessDays:days,dueAt:addBusinessDays(now,days),status:'Asignada',lastUpdatedAt:now,observations:''};try{await saveProjectGovernance({assignment},id);p.assignment=assignment;renderInbox();await renderNotifications();openManageModal(id,true);}catch(e){alert('No fue posible crear la asignación: '+e.message);}}
window.confirmProjectUpdate=confirmProjectUpdate;window.markProjectExecuted=markProjectExecuted;window.assignCurrentProjectWork=assignCurrentProjectWork;
...
function openManageModal(id,skipOneDriveSync=false){
 ...
 manageBody.innerHTML=`<div class="manage-grid">
  <div class="manage-card">
    ...
    <div class="note"><b>Vigencia:</b> ${governanceBadge(p)}<br><b>Ciclo de vida:</b> ${p.lifecycleState||p.state||"—"}${p.assignment?.dueAt?`<br><b>Asignación:</b> ${p.assignment.activity||"Gestión"} · vence ${new Date(p.assignment.dueAt).toLocaleDateString()}`:""}</div><div class="action-row" style="margin:10px 0"><button type="button" class="secondary" onclick="confirmProjectUpdate('${p.id}')">Confirmar actualización</button><button type="button" class="secondary" onclick="assignCurrentProjectWork('${p.id}')">Asignar plazo</button>${p.step>=14&&p.lifecycleState!=="Ejecutado"?`<button type="button" class="btn-approve" onclick="markProjectExecuted('${p.id}')">Marcar como ejecutado</button>`:""}</div>
...
async function manageDecision(id,decision){
 ...
 try{
  ...
  if(!p.rejected&&p.lifecycleState!=='Ejecutado'){const now=new Date().toISOString();const assignment={activity:steps.find(s=>s.n===Number(p.step))?.title||'Gestión del proyecto',responsibleRole:steps.find(s=>s.n===Number(p.step))?.role||p.owner||'',responsible:p.owner||'',institution:p.inst||'',assignedAt:now,businessDays:GOVERNANCE_DEFAULTS.assignmentBusinessDays,dueAt:addBusinessDays(now,GOVERNANCE_DEFAULTS.assignmentBusinessDays),status:'Asignada',lastUpdatedAt:now,observations:comment||''};await saveProjectGovernance({assignment,lifecycleState:p.step>=14?'En ejecución':(p.lifecycleState||p.state)},id);p.assignment=assignment;}

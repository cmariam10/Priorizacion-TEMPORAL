...
async function saveSharedWorkflow(p){
 const f=await resolveSharedFlowFolder(), payload=workflowPayload(p);
 await graphFetch(`/drives/${f.driveId}/items/${f.itemId}:/${encodeURIComponent(safeOneDriveName(p.id)+".json")}:/content`,{
...
function startSharedFlowSync(){
 ...
 sharedFlowSyncTimer=setInterval(async()=>{
  ...
  if(before!==after){renderKpis();renderList();renderMatrix();renderInbox();await renderNotifications();}
 },30000);
}
...
async function writeManagementHistoryDocument(project){
 try{
  ...
  const rows=flow.map((e,i)=>{
   const receivedRole=steps.find(s=>s.n==Number(e.pasoOrigen))?.role||e.perfil||"—";
   const nextRole=steps.find(s=>s.n==Number(e.pasoDestino))?.role||"—";
   return `<tr><td>${i+1}</td><td>${traceEscape(e.fecha?new Date(e.fecha).toLocaleString():"—")}</td><td>${traceEscape(receivedRole)}</td><td>${traceEscape(e.nombreHUB||e.usuarioHUB||"—")}</td><td>${traceEscape(e.perfil||"—")}</td><td>${traceEscape(e.descripcion||"—")}</td><td>${traceEscape(e.comentario||"Sin comentarios")}</td><td>${traceEscape(nextRole)}</td><td>${traceEscape(e.estadoDestino||"—")}</td></tr>`;
  }).join("");
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>Historial de gestión ${traceEscape(project.id)}</title><style>body{font-family:Arial,sans-serif;margin:28px;color:#17233b}h1{color:#0b4d8c}table{border-collapse:collapse;width:100%;font-size:12px}th,td{border:1px solid #cbd5e1;padding:7px;vertical-align:top}th{background:#eef4fa}</style></head><body><h1>Historial de gestión del proyecto</h1><p><b>ID HUB:</b> ${traceEscape(project.id)}<br><b>Proyecto:</b> ${traceEscape(project.name)}<br><b>Actualizado:</b> ${traceEscape(new Date().toLocaleString())}</p><table><thead><tr><th>#</th><th>Fecha de gestión</th><th>Perfil que recibió</th><th>Usuario que gestionó</th><th>Perfil de gestión</th><th>Acción</th><th>Comentarios</th><th>Entregado a</th><th>Estado resultante</th></tr></thead><tbody>${rows||'<tr><td colspan="9">Sin movimientos de gestión.</td></tr>'}</tbody></table></body></html>`;
  const folder=await ensureProjectFolder(project);
  await graphFetch(`/drives/${folder.driveId}/items/${folder.itemId}:/${encodeURIComponent("Historial de gestión - "+safeOneDriveName(project.id)+".html")}:/content`,{method:"PUT",headers:{"Content-Type":"text/html;charset=utf-8"},body:html});
 }catch(e){console.warn("No se pudo actualizar el documento de historial de gestión",e);}
}
...
async function renderNotifications(){
 ...
 const list=projects.filter(p=>allowed.includes(Number(p.step)) || (currentRole==="ejecutor"&&p.step>=10)).slice(0,50);
 internalNotifications=list;
 notifBox.innerHTML=list.length?`<div class="mail-notification-list">${list.map(p=>{
  const s=steps.find(x=>x.n==p.step);
  return `<div class="note" style="border-left:5px solid #0b4d8c;padding:14px 16px;margin-bottom:10px;cursor:pointer" onclick="openManageModal('${p.id}')"><div style="display:flex;justify-content:space-between;gap:16px"><b>📩 Nuevo trabajo · ${traceEscape(p.id)}</b><span class="muted">${p.sharedUpdatedAt?traceEscape(new Date(p.sharedUpdatedAt).toLocaleString()):""}</span></div><div style="margin-top:6px"><b>${traceEscape(p.name)}</b></div><div class="muted" style="margin-top:5px">Asignado a: ${traceEscape(s?.role||p.owner)} · Paso ${p.step}/${TOTAL_WORKFLOW_STEPS}: ${traceEscape(s?.title||"")}</div><div style="margin-top:7px">Abra el proyecto para revisar y gestionar la tarea.</div></div>`;
 }).join("")}</div>`:'<div class="note">No tiene trabajo nuevo asignado a este perfil.</div>';
 const tab=[...document.querySelectorAll(".protected-tab")].find(t=>t.textContent.trim().startsWith("Notificaciones"));
 if(tab){
  let badge=tab.querySelector(".hub-notif-badge");
  if(list.length&&!badge){badge=document.createElement("span");badge.className="hub-notif-badge";badge.style.cssText="margin-left:7px;background:#c62828;color:white;border-radius:999px;padding:2px 7px;font-size:11px;font-weight:700";tab.appendChild(badge);}
  if(badge){badge.textContent=String(list.length);badge.style.display=list.length?"inline-block":"none";}
 }
}
...
async function manageDecision(id,decision){
 ...
 openManageModal(id);
 alert("Gestión registrada y sincronizada. El siguiente perfil ya puede verla en su bandeja y notificaciones.");
}

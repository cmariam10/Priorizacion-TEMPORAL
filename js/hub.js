...
// V29.1: sin indicador flotante global. Los estados de espera se muestran en la acción correspondiente.
...
function renderAdminGovernance(){
 ...
 <div class="filter-panel"><div><label>Buscar</label><input id="adminSearch" value="${adminEsc(existing.q)}" oninput="renderAdminProjectRows()" placeholder="ID, proyecto, institución, responsable..."/></div><div><label>Vigencia</label><select id="adminFreshFilter" onchange="renderAdminProjectRows()"><option value="">Todas</option><option value="current">Vigente</option><option value="warning">Próximo a actualizar</option><option value="update_required">Requiere actualización</option><option value="requested">Actualización solicitada</option></select></div><div><label>Plazo del paso</label><select id="adminDeadlineFilter" onchange="renderAdminProjectRows()"><option value="">Todos</option><option value="ok">En plazo</option><option value="warning">Próximo a vencer</option><option value="overdue">Vencido</option><option value="none">Sin plazo</option></select></div><div><label>Institución</label><select id="adminInstFilter" onchange="renderAdminProjectRows()"><option value="">Todas</option>${insts.map(x=>`<option>${adminEsc(x)}</option>`).join('')}</select></div></div>

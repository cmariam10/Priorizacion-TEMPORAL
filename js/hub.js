...
function businessDaysBetween(a,b){let d=new Date(a),end=new Date(b),n=0;if(!Number.isFinite(d.getTime())||!Number.isFinite(end.getTime()))return null;d.setHours(12,0,0,0);end.setHours(12,0,0,0);while(d<end){d.setDate(d.getDate()+1);const w=d.getDay();if(w!==0&&w!==6)n++;}return n;}
function addBusinessDays(date,days){let d=new Date(date);if(!Number.isFinite(d.getTime()))d=new Date();let n=Math.max(0,Number(days)||0);while(n>0){d.setDate(d.getDate()+1);if(d.getDay()!==0&&d.getDay()!==6)n--;}return d.toISOString();}
function excelUpdatedAt(p){const d=p?.excelData||{};return d['Fecha actualización']||d['Fecha Actualización']||d['Fecha actualizacion']||'';}
function deriveFreshness(p){const last=p.governance?.lastValidatedAt||excelUpdatedAt(p)||p.sharedUpdatedAt||'';const days=businessDaysBetween(last,new Date());if(days===null)return {code:'unknown',label:'Sin fecha de actualización',days:null,last};const max=Number(p.governance?.maxBusinessDays||governanceConfig.maxBusinessDays||GOVERNANCE_DEFAULTS.maxBusinessDays),warn=Number(p.governance?.warningBusinessDays||governanceConfig.warningBusinessDays||GOVERNANCE_DEFAULTS.warningBusinessDays);if(p.governance?.updateRequest?.status==='Pendiente')return {code:'requested',label:'Actualización solicitada',days,last};if(days>max)return {code:'update_required',label:'Requiere actualización',days,last};if(days>=Math.max(0,max-warn))return {code:'warning',label:'Próximo a actualizar',days,last};return {code:'current',label:'Vigente',days,last};}
...
function adminDateValue(v){const t=v?new Date(v).getTime():0;return Number.isFinite(t)?t:0;}
...
function adminFmtDate(v){if(!v)return '—';const d=new Date(v);return Number.isNaN(d.getTime())?'—':d.toLocaleDateString();}
...
function adminProjectLastUpdate(p){return p.governance?.lastValidatedAt||excelUpdatedAt(p)||p.sharedUpdatedAt||'';}

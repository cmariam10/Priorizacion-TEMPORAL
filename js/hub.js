...
async function appendExcelProject(values){
 ...
 // Microsoft Graph Excel API: agregar una fila al final de la tabla.
 return graphFetch(`/drives/${b.driveId}/items/${b.itemId}/workbook/tables/${encodeURIComponent(t.name)}/rows/add`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({values:[values]})});
}
async function patchExcelRow(index,values){
 const b=await resolveExcelWorkbook(),t=await resolveExcelTable();
 // Actualiza la fila por su índice cero-basado dentro de la tabla.
 return graphFetch(`/drives/${b.driveId}/items/${b.itemId}/workbook/tables/${encodeURIComponent(t.name)}/rows/${Number(index)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({values:[values]})});
}

...
function loadLeaflet(){
 return new Promise((resolve,reject)=>{
  ...
  if(!document.getElementById("leaflet-css")){const c=document.createElement("link");c.id="leaflet-css";c.rel="stylesheet";c.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css";document.head.appendChild(c);}
  let s=document.getElementById("leaflet-js");
  if(s){s.addEventListener("load",()=>resolve(window.L),{once:true});s.addEventListener("error",reject,{once:true});return;}
  s=document.createElement("script");s.id="leaflet-js";s.src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js";s.onload=()=>resolve(window.L);s.onerror=()=>reject(new Error("No fue posible cargar el mapa interactivo."));document.head.appendChild(s);
 });
}
...
function updateGeometryStatus(){
 ...
 else{const n=g.referencePoints?.length||0, mode=g.routeMode==="road"?"por carretera":g.routeMode==="linear"?"lineal":"pendiente";b.innerHTML=`<b>Tramo:</b> ${n} punto${n===1?"":"s"} de referencia · trazado ${mode}.`;}
}
 ...
 try{
  ...
  registrationMap.on("click",e=>{
   ...
   if(t==="point")pendingProjectGeometry={type:"point",source:"user",referencePoints:[{lat:e.latlng.lat,lon:e.latlng.lng}]};
    ...
    pendingProjectGeometry.coordinates=pendingProjectGeometry.referencePoints.map(p=>[p.lon,p.lat]);pendingProjectGeometry.routeMode="linear";
   }
...
function changeLocationType(){const t=document.getElementById("locationType")?.value;pendingProjectGeometry=t==="point"?{type:"point",source:"user",referencePoints:[]}:t==="segment"?{type:"segment",source:"user",referencePoints:[],coordinates:[],routeMode:null}:null;redrawRegistrationGeometry();}
function undoLocationPoint(){const g=pendingProjectGeometry;if(!g)return;if(g.type==="point")g.referencePoints=[];else{g.referencePoints.pop();g.coordinates=g.referencePoints.map(p=>[p.lon,p.lat]);g.routeMode="linear";}redrawRegistrationGeometry();}
...
async function routeProjectSegment(){
 const g=pendingProjectGeometry;if(!g||g.type!=="segment"||g.referencePoints.length<2){alert("Marque al menos dos puntos de referencia.");return;}
 const b=document.getElementById("geometryStatus");if(b)b.innerHTML="Buscando conexión por la carretera existente…";
 try{
  const coords=g.referencePoints.map(p=>`${p.lon},${p.lat}`).join(";");
  const r=await fetch(`https://router.project-osrm.org/route/v1/driving/${coords}?overview=full&geometries=geojson`);
  if(!r.ok)throw 0;const d=await r.json();if(!d.routes?.[0]?.geometry?.coordinates?.length)throw 0;
  g.coordinates=d.routes[0].geometry.coordinates;g.routeMode="road";g.routeDistanceMeters=d.routes[0].distance||null;
 }catch(e){g.coordinates=g.referencePoints.map(p=>[p.lon,p.lat]);g.routeMode="linear";alert("No se encontró una ruta vial que conecte los puntos. Se usará un trazado lineal.");}
 redrawRegistrationGeometry();
}
window.changeLocationType=changeLocationType;window.undoLocationPoint=undoLocationPoint;window.clearProjectLocation=clearProjectLocation;window.routeProjectSegment=routeProjectSegment;
...
function renderRegistrationForm(data={}){
 ...
 box.insertAdjacentHTML("beforeend",`<div class="location-editor">
  ...
  <div class="location-controls"><div><label>Tipo de ubicación *</label><select id="locationType" onchange="changeLocationType()"><option value="">Seleccione…</option><option value="point">Puntual</option><option value="segment">Tramo</option></select></div><div class="location-help"><b>Puntual:</b> marque un punto.<br><b>Tramo:</b> marque dos o más referencias en orden. El HUB intentará seguir la carretera existente y, si no encuentra conexión, usará una línea directa.</div></div>
  <div id="projectLocationMap" class="project-location-map"></div><div id="geometryStatus" class="note"></div>
  <div class="action-row location-actions"><button type="button" onclick="routeProjectSegment()">Trazar tramo por carretera</button><button type="button" class="secondary" onclick="undoLocationPoint()">Deshacer último punto</button><button type="button" class="secondary" onclick="clearProjectLocation()">Limpiar ubicación</button></div>
 </div>`);
...
async function initFichaInteractiveMap(geo){
 ...
 try{
  ...
  if(g?.type==="segment"&&g.coordinates?.length>1){const line=L.polyline(g.coordinates.map(c=>[c[1],c[0]]),{weight:6}).addTo(fichaLeafletMap);g.referencePoints.forEach((p,i)=>L.marker([p.lat,p.lon]).addTo(fichaLeafletMap).bindTooltip(`Referencia ${i+1}`));fichaLeafletMap.fitBounds(line.getBounds().pad(.15));}
...
async function addProject(){
 ...
 try{
  ...
  if(pendingProjectGeometry.type==="segment"&&pendingProjectGeometry.routeMode!=="road")await routeProjectSegment();

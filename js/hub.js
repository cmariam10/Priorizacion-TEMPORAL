...
function renderAdminProjectRows(){const body=document.getElementById('adminProjectRows');if(!body)return;const arr=adminFilteredProjects();body.innerHTML=arr.slice(0,500).map(p=>{const fr=deriveFreshness(p),dl=adminDeadlineState(p),last=adminProjectLastUpdate(p),req=p.governance?.updateRequest;return `<tr><td><b>${adminEsc(p.id)}</b></td><td>${adminEsc(p.name)}<br><span class="muted">${adminEsc(p.inst||'')}</span></td><td>${Number(p.step||0)}. ${adminEsc(steps.find(s=>s.n==p.step)?.title||'')}</td><td>${adminFmtDate(last)}${fr.days!=null?`<br><span class="muted">${fr.days} días hábiles</span>`:''}</td><td>${governanceBadge(p)}${req?.status==='Pendiente'?`<br><span class="muted">Solicitud vence ${adminFmtDate(req.dueAt)}</span>`:''}</td><td>${adminFmtDate(p.assignment?.dueAt)}<br><span class="muted">${adminEsc(dl.label)}</span></td><td><div class="action-row"><button class="secondary" onclick="openManageModal('${adminEsc(p.id)}')">Ver ficha</button><button onclick="requestProjectUpdate('${adminEsc(p.id)}')" ${req?.status==='Pendiente'?'disabled title="Ya existe una solicitud pendiente"':''}>Solicitar actualización</button>${p.assignment?.extension?.status==='Solicitada'?`<button class="btn-approve" onclick="decideExtension('${adminEsc(p.id)}',true)">Prórroga</button>`:''}</div></td></tr>`}).join('')||'<tr><td colspan="7">No hay proyectos que coincidan con los filtros.</td></tr>';}
...
window.renderAdminGovernance=renderAdminGovernance;window.renderAdminProjectRows=renderAdminProjectRows;window.adminSetSort=adminSetSort;window.adminEditStepDeadline=adminEditStepDeadline;

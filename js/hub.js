...
function renderCustomFilterBuilder(){
 ...
 <div>${customPortfolioFilters.map((f,i)=>{
   ...
   return `<div class="custom-filter-row">
    <select onchange="setCustomFilterField(${i},this.value)"><option value="">Seleccione una variable…</option>${fields.map(k=>`<option value="${traceEscape(k)}" ${f.field===k?"selected":""}>${traceEscape(k)}</option>`).join("")}</select>

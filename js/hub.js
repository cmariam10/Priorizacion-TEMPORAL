 ...
 projects=rows.map((raw,index)=>{
  ...
  const pick=(...names)=>{
   for(const n of names){
    if(data && data[n]!==undefined && data[n]!==null && String(data[n]).trim()!=="")return data[n];
    if(raw && raw[n]!==undefined && raw[n]!==null && String(raw[n]).trim()!=="")return raw[n];
   }
   return "";
  };
  ...
  return {
   id,
   name:pick("Nombre del proyecto","name","nombreProyecto","Nombre corto")||"Sin nombre",
   inst:pick("Institución","inst","institution","institucion")||"Sin institución",
   step:Number(pick("step","paso"))||1,
   state:pick("Estado","state","estado")||"Borrador",
   priority:pick("Estado (PREVIO, REVISAR)","Estado previo","priority")||"",
   bpip:pick("Código del BPIP","bpip","codigoBPIP")||"Por definir",
   categoria:pick("Tipo de Obra","categoria","tipoObra")||"",
   subcategoria:pick("Sub tipo de obra","subcategoria","subTipoObra")||"",
   etapa:pick("Etapa","etapa")||"Idea",
   owner:pick("owner","responsable")||"Técnico / Profesional",
   returned:!!pick("returned","devuelto"),
   rejected:!!pick("rejected","rechazado"),
   excelIndex:index,
   excelData:data||{}
  };
 }).filter(p=>p.id);
...
async function loginRole(){
 ...
 try{
   ...
   try{
     ...
     // Migración segura: la base maestra ya se carga exclusivamente por el Worker.
     // El flujo compartido directo por Graph queda pausado hasta exponer su endpoint seguro en el Worker.

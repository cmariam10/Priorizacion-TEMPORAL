...
async function obtenerIdsRepositorioMOPT(){
 await initMicrosoft();
 const acct=msalApp.getActiveAccount()||msalApp.getAllAccounts()[0];
 if(!acct)throw new Error("Primero inicia sesión con la cuenta propietaria de Microsoft.");

 const root=await graphFetch("/me/drive/root:/HUB_Proyectos_MOPT?$select=id,name,parentReference,webUrl");
 const driveId=root?.parentReference?.driveId;
 if(!driveId||!root?.id)throw new Error("No pude localizar HUB_Proyectos_MOPT en la raíz del OneDrive de esta cuenta.");

 // Search the owner's drive for the Excel master, avoiding the problematic /shares endpoint.
 const q=encodeURIComponent("HUB_Proyectos");
 const found=await graphFetch(`/me/drive/root/search(q='${q}')?$select=id,name,parentReference,webUrl,file`);
 const candidates=(found?.value||[]).filter(x=>/\.xlsx$/i.test(x.name||""));
 let excel=candidates.find(x=>/hub[_ ]?proyectos/i.test(x.name||""))||candidates[0]||null;

 const info={
  cuenta:acct.username||"",
  carpeta:{
   nombre:root.name,
   driveId,
   itemId:root.id,
  ...
  excel:excel?{
   nombre:excel.name,
   driveId:excel.parentReference?.driveId||driveId,
   itemId:excel.id,
   webUrl:excel.webUrl||""
  }:null
 };
 window.__HUB_REPOSITORY_IDS__=info;

 const msg=[
  "IDENTIFICADORES DEL REPOSITORIO MOPT",
  "",
  `Cuenta: ${info.cuenta}`,
  "",
  "CARPETA HUB_Proyectos_MOPT",
  `driveId: ${info.carpeta.driveId}`,
  `itemId: ${info.carpeta.itemId}`,
  "",
  "EXCEL MAESTRO",
  excel?`Archivo: ${info.excel.nombre}`:"No se encontró automáticamente.",
  excel?`driveId: ${info.excel.driveId}`:"",
  excel?`itemId: ${info.excel.itemId}`:"",
  "",
  "Copia o toma una captura de este mensaje."
 ].filter(Boolean).join("\n");
 alert(msg);
 return info;
}

function instalarBotonIdsRepositorio(){
 if(document.getElementById("hubGetRepositoryIds"))return;
 const btn=document.createElement("button");
 btn.id="hubGetRepositoryIds";
 btn.type="button";
 btn.textContent="Obtener IDs OneDrive";
 btn.title="Diagnóstico temporal para configurar el repositorio central";
 btn.style.cssText="position:fixed;right:18px;bottom:18px;z-index:99999;padding:11px 15px;border:0;border-radius:10px;background:#0b3558;color:#fff;font-weight:700;box-shadow:0 4px 16px #0003;cursor:pointer";
 btn.addEventListener("click",async()=>{
  btn.disabled=true;
  const old=btn.textContent;
  btn.textContent="Consultando...";
  try{await obtenerIdsRepositorioMOPT();}
  catch(e){alert("No fue posible obtener los IDs: "+(e?.message||e));}
  finally{btn.disabled=false;btn.textContent=old;}
 });
 document.body.appendChild(btn);
}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",instalarBotonIdsRepositorio);
else instalarBotonIdsRepositorio();

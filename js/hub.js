...
window.HUB_ONEDRIVE_BUILD = "2026-09-16b";
...
async function writeTrace(project,action,documentName,extra={}){
 try{
  ...
  const filename=`${new Date().toISOString().replace(/[:.]/g,"-")}_${safeOneDriveName(project.id)}_${Math.random().toString(36).slice(2,8)}.json`;
  await graphFetch(`/drives/${root.driveId}/items/${traceFolder.id}:/${encodeURIComponent(filename)}:/content`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(event,null,2)});
...
async function deleteOneDriveDocument(index){
 ...
 try{const folder=await ensureProjectFolder(p);await graphFetch(`/drives/${folder.driveId}/items/${d.itemId}`,{method:"DELETE"});const oldName=d.name;documents.splice(index,1);auditLog.unshift({project:p.id,action:"Eliminó documento",role:actorLabel(),date:new Date().toLocaleString(),comment:`Se eliminó ${oldName} de OneDrive.`});saveHubState();await writeTrace(p,"ELIMINO_DOCUMENTO",oldName,{itemId:d.itemId});renderDocuments();openManageModal(p.id,true);alert("Documento eliminado de OneDrive. La trazabilidad se conservó.");}catch(e){console.error(e);alert("No fue posible eliminar el documento de OneDrive: "+e.message);}
}

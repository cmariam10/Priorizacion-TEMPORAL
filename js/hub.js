...
async function appendExcelProject(values){const b=await resolveExcelWorkbook(),t=await resolveExcelTable();return graphFetch(`/drives/${b.driveId}/items/${b.itemId}/workbook/tables/${encodeURIComponent(t.name)}/rows`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({values:[values]})});}
async function patchExcelRow(index,values){const b=await resolveExcelWorkbook(),t=await resolveExcelTable();return graphFetch(`/drives/${b.driveId}/items/${b.itemId}/workbook/tables/${encodeURIComponent(t.name)}/rows/${index}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({values:[values]})});}
...
async function syncProjectDocuments(id){
 ...
 documents=documents.filter(d=>d.project!==id||(!d.itemId&&!d.webUrl));
 remote.forEach(r=>{const demo=documents.find(d=>d.project===id&&d.name===r.name&&!d.itemId); if(demo)Object.assign(demo,r); else documents.push(r);});

...
const steps=[
{n:1,title:"Registro de iniciativa",phase:1,role:"Proponente / Formulador",desc:"Registra la iniciativa, información básica y soportes iniciales."},
{n:2,title:"Revisión de integridad",phase:1,role:"Revisor institucional",desc:"Verifica completitud, consistencia y requisitos mínimos."},
{n:3,title:"Alineación institucional",phase:1,role:"Revisor institucional",desc:"Revisa alineación con planes, prioridades y competencia institucional."},
{n:4,title:"Aprobación institucional",phase:1,role:"Revisor institucional",desc:"Aprueba institucionalmente la iniciativa o la devuelve con observaciones."},
{n:5,title:"Habilitación para evaluación",phase:1,role:"Revisor institucional",desc:"Confirma soportes mínimos y habilita la iniciativa para la evaluación central."},
{n:6,title:"Revisión y clasificación",phase:2,role:"Secretaría Técnica / SPS",desc:"Clasifica la iniciativa, confirma metodología y asigna la evaluación técnica."},
{n:7,title:"Evaluación técnica de priorización",phase:2,role:"Técnico evaluador / Validador especializado",desc:"El técnico diligencia variables y soportes; el validador especializado revisa y aprueba la evaluación."},
{n:8,title:"Cierre técnico y cálculo",phase:2,role:"Secretaría Técnica / SPS",desc:"Verifica completitud, ejecuta o recalcula resultados y consolida el ranking."},
{n:9,title:"Priorización y decisión",phase:2,role:"Tomador de decisión / Comité",desc:"Revisa ranking, evidencia y trazabilidad para registrar la decisión."},
{n:10,title:"Formulación definitiva",phase:3,role:"Proponente / Formulador",desc:"Continúa la preparación de la iniciativa priorizada conforme al proceso aplicable."},
{n:11,title:"Registro como proyecto",phase:3,role:"Proponente / Formulador",desc:"Formaliza el proyecto aprobado y completa la información requerida."},
{n:12,title:"Asignación de responsables",phase:3,role:"Proponente / Formulador",desc:"Define equipo, unidad responsable y responsables de ejecución."},
{n:13,title:"Planificación de ejecución",phase:3,role:"Proponente / Formulador",desc:"Completa cronograma, hitos y planificación de ejecución."},
{n:14,title:"Ejecución",phase:4,role:"Proponente / Formulador",desc:"Registra el inicio y avance de la ejecución."},
{n:15,title:"Seguimiento y control",phase:4,role:"Proponente / Formulador",desc:"Reporta avance, riesgos e hitos de seguimiento."},
{n:16,title:"Supervisión / interventoría",phase:4,role:"Proponente / Formulador",desc:"Registra verificaciones de calidad y cumplimiento."},
{n:17,title:"Cierre del proyecto",phase:4,role:"Proponente / Formulador",desc:"Registra el cierre y las lecciones aprendidas."}
];
const BASE_PROJECTS = [];
...
// Perfiles funcionales del HUB. La administración del sistema queda fuera de la cadena de aprobación.
const canRegisterRoles=["proponente","admin"];
const roleStepPermissions={
 proponente:[1,10,11,12,13,14,15,16,17],
 revisor:[2,3,4,5],
 tecnico:[7],
 validador:[7],
 secretaria:[6,8],
...
const docEditPermissions={
 proponente:["Ficha de iniciativa","Diagnóstico / justificación","Estimación de costos","Plan de ejecución"],
 revisor:["Ficha de iniciativa","Aval institucional"],
 tecnico:["Ficha de iniciativa","Diagnóstico / justificación","Concepto técnico"],
 validador:["Concepto técnico","Concepto financiero"],
 secretaria:["Concepto técnico","Concepto financiero","Plan de ejecución","Matriz de criterios"],
 decision:[],
 admin:["Ficha de iniciativa","Diagnóstico / justificación","Estimación de costos","Aval institucional","Concepto técnico","Concepto financiero","Plan de ejecución","Matriz de criterios"]
};
let documents=[];
...
const ROLE_DISPLAY_STEPS={proponente:"1 y 10–17",revisor:"2–5",tecnico:"7",validador:"7",secretaria:"6 y 8",decision:"9",admin:"—"};
...
function isProjectAssignedToRole(p,role=currentRole){
 if(!p||role==="visitor"||role==="admin")return false;
 const step=Number(p.step)||1;
 const state=String(p.state||"");
 if(role==="proponente")return step===1||step>=10;
 if(role==="revisor")return [2,3,4,5].includes(step);
 if(role==="secretaria")return [6,8].includes(step);
 if(role==="decision")return step===9;
 if(role==="tecnico")return step===7 && !/pendiente validaci[oó]n especializada/i.test(state);
 if(role==="validador")return step===7 && /pendiente validaci[oó]n especializada/i.test(state);
 return false;
}

function notificationProjects(){
 if(!currentUser||currentRole==="visitor")return [];
 return projects.filter(p=>isProjectAssignedToRole(p,currentRole)&&p.sharedUpdatedAt);
}
...
function refreshWorkBadges(){
 ...
 if(currentUser&&currentRole!=="visitor"){
  pending=projects.filter(p=>isProjectAssignedToRole(p,currentRole)).length;
 }
...
async function graphFetch(path,options={}){
 ...
 // Cuando el recurso pertenece al OneDrive del usuario autenticado usamos /me/drive.
 // Esto evita el error ObjectHandle is Invalid que puede producir Graph con /drives/{id}
 // en OneDrive for Business/SharePoint personal sites.
 let normalizedPath=String(path||"");
 normalizedPath=normalizedPath.replace(/^\/drives\/ME\/items\//,"/me/drive/items/");
 const url=normalizedPath.startsWith("http")?normalizedPath:`https://graph.microsoft.com/v1.0${normalizedPath}`;
 ...
 if(!res.ok){
  ...
  const shortPath=String(normalizedPath).replace(/^https:\/\/graph\.microsoft\.com\/v1\.0/,"");
...
async function resolveExcelWorkbook(){
 ...
 // 1) Ruta canónica dentro del OneDrive del usuario autenticado.
 // Para este repositorio, /me/drive/root corresponde a la biblioteca Documents
 // que contiene HUB_Proyectos_MOPT. No depende de driveId ni de vínculos compartidos.
 try{
  const item=await graphFetch(`/me/drive/root:/HUB_Proyectos_MOPT/HUB_Proyectos.xlsx?$select=id,name,webUrl,parentReference,file`);
  if(!item?.id||!item?.file)throw new Error("No se encontró HUB_Proyectos.xlsx en HUB_Proyectos_MOPT.");
  excelBook={driveId:"ME",itemId:item.id,name:item.name,webUrl:item.webUrl};
  return excelBook;
 }catch(pathErr){
  console.warn("No se pudo resolver HUB_Proyectos.xlsx por /me/drive; se intentará el vínculo compartido.",pathErr);
 }

 // 2) Respaldo por vínculo compartido.
 try{
  const item=await resolveSharedDriveItem(MS_CONFIG.excelShareUrl);
  const driveId=item?.parentReference?.driveId;
  if(!driveId||!item?.id)throw new Error("No fue posible resolver el Excel maestro desde el vínculo compartido.");
  excelBook={driveId,itemId:item.id,name:item.name,webUrl:item.webUrl};
  return excelBook;
 }catch(sharedErr){
  console.warn("No se pudo resolver el Excel por vínculo compartido; se intentará la referencia directa.",sharedErr);
 }

 // 3) Último respaldo con los identificadores configurados.
...
async function resolveRootFolder(){
 ...
 // 1) Resolver por ruta dentro del OneDrive del usuario autenticado.
 // Es la vía más estable y evita depender de ObjectHandle/driveId compartidos.
 try{
  const item=await graphFetch(`/me/drive/root:/HUB_Proyectos_MOPT?$select=id,name,webUrl,parentReference,folder`);
  if(!item?.folder||!item?.id)throw new Error("No se encontró la carpeta HUB_Proyectos_MOPT en el OneDrive autenticado.");
  graphRoot={driveId:"ME",itemId:item.id,name:item.name,webUrl:item.webUrl};
  return graphRoot;
 }catch(pathErr){
  console.warn("No se pudo resolver HUB_Proyectos_MOPT por /me/drive; se intentará el vínculo compartido.",pathErr);
 }

 // 2) Respaldo por vínculo compartido.
 try{
  const item=await resolveSharedDriveItem(MS_CONFIG.rootShareUrl);
  const driveId=item?.parentReference?.driveId;
  if(!item?.folder||!driveId||!item?.id)throw new Error("HUB_Proyectos_MOPT no corresponde a una carpeta compartida accesible.");
  graphRoot={driveId,itemId:item.id,name:item.name,webUrl:item.webUrl};
  return graphRoot;
 }catch(sharedErr){
  console.warn("No se pudo resolver la carpeta raíz por vínculo compartido; se intentará la referencia directa.",sharedErr);
 }

 // 3) Último respaldo por IDs fijos.
...
const roleInfo={
 proponente:["Registra iniciativas","Completa información básica","Adjunta documentos y soportes","Atiende solicitudes de información","Corrige iniciativas devueltas"],
 revisor:["Verifica integridad y consistencia del registro","Revisa requisitos mínimos","Aprueba el avance institucional","Devuelve con observaciones cuando se requieran ajustes"],
 tecnico:["Diligencia variables manuales de priorización","Selecciona rangos y categorías","Revisa resultados automáticos","Adjunta evidencia técnica","Atiende observaciones especializadas"],
 validador:["Revisa variables técnicas diligenciadas","Verifica documentos y fuentes","Valida resultados automáticos","Aprueba la evaluación especializada","Devuelve al técnico con observaciones específicas"],
 secretaria:["Administra la metodología de priorización","Gestiona matrices, criterios y ponderaciones","Verifica la completitud de las evaluaciones","Ejecuta o recalcula resultados","Consolida y genera el ranking"],
 decision:["Consulta el ranking de iniciativas","Compara puntajes y resultados","Revisa evidencia y trazabilidad","Registra la decisión","Solicita aclaraciones cuando corresponda"],
 admin:["Administra usuarios y perfiles","Configura permisos","Gestiona parámetros generales del HUB","Supervisa integraciones y funcionamiento técnico","No interviene en la evaluación o decisión"]
};
...
function showView(id,btn){
 if(["bandeja","notificaciones","registrar","documentos"].includes(id)&&currentRole==="visitor"){
   alert("Para acceder a este módulo debe iniciar sesión seleccionando un perfil.");
   return;
 }
...
function previewRole(){
 ...
 const roleName=`${baseRoleName} · pasos ${ROLE_DISPLAY_STEPS[currentRole]||"—"}`;
...
function refreshRoleAccess(){
 ...
 if(roleEl)[...roleEl.options].forEach(o=>{if(!o.value)return;if(!o.dataset.baseLabel)o.dataset.baseLabel=o.textContent.replace(/\s*\([^)]*\)\s*$/,"" );o.textContent=`${o.dataset.baseLabel} · pasos ${ROLE_DISPLAY_STEPS[o.value]||"—"}`;});
...
async function changeTestProfile(){
 ...
 if(!roleEl)return;
 currentRole=roleEl.value||"proponente";
...
async function loginRole(){
 ...
 try{
   ...
   // Forzar resolución fresca de OneDrive en cada inicio de sesión.
   excelBook=null; excelTable=null; excelHeaders=[]; graphRoot=null; sharedFlowFolder=null;
   loadHubState();
   ...
   try{
     ...
     await loadAllSharedWorkflow();
     await recoverLegacyWorkflowFromTrace();
     await loadAllSharedWorkflow();
     startSharedFlowSync();
   }catch(excelErr){console.error(excelErr);alert("Sesión iniciada, pero no fue posible cargar/sincronizar la base maestra y el flujo compartido. Verifique que la sesión Microsoft corresponda a la cuenta que tiene acceso a HUB_Proyectos_MOPT. Detalle: "+excelErr.message);}
...
function canManageProject(p){
 if(currentRole==="visitor"||currentRole==="admin")return false;
 return isProjectAssignedToRole(p,currentRole);
}
...
function openManageModal(id,skipOneDriveSync=false){
 ...
 let title="Estado actual", placeholder="Escriba la justificación, observación o instrucción para el siguiente responsable", actions="";
 let quick="La gestión queda registrada en la trazabilidad. Cada perfil actúa únicamente sobre la etapa que le corresponde.";

 if(currentRole==="proponente"){
  title=p.step===1?"Registro / ajuste de iniciativa":"Continuidad de la iniciativa";
  placeholder="Describa el registro, complemento o ajuste realizado";
  actions=p.step===1?`
   <button class="btn-approve" onclick="manageDecision('${p.id}','enviar_revision')">Enviar a revisión institucional</button>
   <button class="btn-purple" onclick="manageDecision('${p.id}','guardar_borrador')">Guardar borrador / ajuste</button>`:`
   <button class="btn-approve" onclick="manageDecision('${p.id}','aprobar')">Completar paso / avanzar</button>
   <button class="btn-purple" onclick="manageDecision('${p.id}','guardar_borrador')">Guardar avance</button>`;
  quick="El Proponente / Formulador registra la iniciativa, atiende devoluciones y continúa la información posterior cuando corresponda. No aprueba su propio registro.";
 }else if(currentRole==="revisor"){
  actions=`<button class="btn-approve" onclick="manageDecision('${p.id}','aprobar')">Aprobar / avanzar</button><button class="btn-return" onclick="manageDecision('${p.id}','devolver')">Devolver al proponente</button><button class="btn-reject" onclick="manageDecision('${p.id}','rechazar')">Rechazar / archivar</button>`;
  quick="El Revisor institucional verifica integridad, alineación y requisitos. Si devuelve, la iniciativa regresa al Proponente / Formulador con las observaciones registradas.";
 }else if(currentRole==="tecnico"){
  title="Evaluación técnica de priorización";
  actions=`<button class="btn-approve" onclick="manageDecision('${p.id}','enviar_validacion')">Enviar a validación especializada</button><button class="btn-purple" onclick="manageDecision('${p.id}','guardar_borrador')">Guardar evaluación en curso</button>`;
  quick="El Técnico evaluador diligencia variables de priorización y soportes. No aprueba su propia evaluación: la remite al Validador especializado.";
 }else if(currentRole==="validador"){
  title="Validación especializada";
  actions=`<button class="btn-approve" onclick="manageDecision('${p.id}','aprobar')">Validar y enviar a SPS</button><button class="btn-return" onclick="manageDecision('${p.id}','devolver')">Devolver al técnico</button>`;
  quick="El Validador especializado revisa variables, fuentes y soportes. Puede aprobar la evaluación o devolverla al Técnico evaluador con observaciones específicas.";
 }else if(currentRole==="secretaria"){
  title=p.step===8?"Cierre técnico de priorización":"Revisión y clasificación";
  actions=`<button class="btn-approve" onclick="manageDecision('${p.id}','aprobar')">${p.step===8?"Cerrar evaluación y enviar a decisión":"Aprobar / avanzar"}</button><button class="btn-return" onclick="manageDecision('${p.id}','devolver')">Devolver para ajustes</button>`;
  quick="La Secretaría Técnica / SPS administra la metodología, verifica la evaluación y consolida los resultados. En el paso 8 cierra la evaluación y remite el ranking al Tomador de decisión.";
 }else if(currentRole==="decision"){
  title="Decisión sobre la iniciativa";
  actions=`<button class="btn-approve" onclick="manageDecision('${p.id}','priorizar')">Priorizar iniciativa</button><button class="btn-return" onclick="manageDecision('${p.id}','aclaracion')">Solicitar aclaración</button><button class="btn-purple" onclick="manageDecision('${p.id}','banco')">Mantener en banco</button>`;
  quick="El Tomador de decisión consulta el ranking, puntajes, evidencia y trazabilidad. No modifica la evaluación técnica; registra la decisión o solicita aclaraciones.";
 }
 ...
 manageBody.innerHTML=`<div class="manage-grid">
  <div class="manage-card">
    ...
    ${allowed?`<label>Observación de gestión</label><textarea id="manageComment" rows="4" placeholder="${placeholder}"></textarea><div class="action-row">${actions}</div>`:`<div class="note">Su perfil actual no tiene una acción operativa en este paso. Puede consultar el historial y los documentos disponibles.</div>`}
  </div>
  <div class="manage-card">
...
async function manageDecision(id,decision){
 const p=projects.find(x=>x.id==id); if(!p)return;
 const pasoOrigen=Number(p.step)||1, estadoOrigen=p.state;
 ...
 if(!canManageProject(p)){failHubOperationProgress("Sin permiso para gestionar este paso");alert("Su perfil no tiene permiso para gestionar este paso.");return;}
 let action="";

 if(currentRole==="proponente"){
  if(pasoOrigen===1 && decision==="enviar_revision"){
   p.step=2;p.state="En revisión institucional";p.owner="Revisor institucional";p.returned=false;p.rejected=false;
   action="Envió la iniciativa a revisión institucional";
  }else if(decision==="guardar_borrador"){
   p.state=pasoOrigen===1?"Borrador / ajuste en curso":"Información en actualización";p.owner="Proponente / Formulador";p.returned=false;p.rejected=false;
   action=pasoOrigen===1?"Guardó el registro o ajuste de la iniciativa":"Guardó avances de información";
  }else if(pasoOrigen>=10 && decision==="aprobar"){
   if(p.step<17)p.step+=1;
   p.state=p.step>=14?"En ejecución":"En preparación";p.owner=steps.find(s=>s.n==p.step)?.role||"Proponente / Formulador";p.returned=false;p.rejected=false;
   action="Completó el paso y avanzó la iniciativa";
  }else{
   failHubOperationProgress("Acción no permitida");alert("El Proponente / Formulador registra, ajusta y envía información. No aprueba su propio registro.");return;
  }
 }
 else if(currentRole==="revisor"){
  if(decision==="aprobar"){
   if(p.step<5){p.step+=1;p.state="En revisión institucional";p.owner="Revisor institucional";}
   else{p.step=6;p.state="Pendiente revisión y clasificación";p.owner="Secretaría Técnica / SPS";}
   p.returned=false;p.rejected=false;action="Aprobó la revisión institucional y avanzó la iniciativa";
  }else if(decision==="devolver"){
   p.step=1;p.state="Devuelto al proponente para ajustes";p.owner="Proponente / Formulador";p.returned=true;p.rejected=false;action="Devolvió la iniciativa al Proponente / Formulador con observaciones";
  }else if(decision==="rechazar"){
   p.state="Rechazado / archivado";p.rejected=true;p.returned=false;action="Rechazó y archivó la iniciativa";
  }else{failHubOperationProgress("Acción no permitida");return;}
 }
 else if(currentRole==="secretaria"){
  if(decision==="aprobar" && pasoOrigen===6){
   p.step=7;p.state="Pendiente evaluación técnica";p.owner="Técnico evaluador";p.returned=false;p.rejected=false;action="Clasificó la iniciativa y la remitió a evaluación técnica";
  }else if(decision==="aprobar" && pasoOrigen===8){
   p.step=9;p.state="Disponible para decisión";p.owner="Tomador de decisión / Comité";p.returned=false;p.rejected=false;action="Cerró la evaluación y remitió el ranking para toma de decisión";
  }else if(decision==="devolver" && pasoOrigen===6){
   p.step=5;p.state="Devuelto para ajuste institucional";p.owner="Revisor institucional";p.returned=true;p.rejected=false;action="Devolvió la iniciativa al Revisor institucional para ajustes";
  }else if(decision==="devolver" && pasoOrigen===8){
   p.step=7;p.state="Devuelto al técnico para ajustes";p.owner="Técnico evaluador";p.returned=true;p.rejected=false;action="Devolvió la evaluación al Técnico evaluador para ajustes";
  }else{failHubOperationProgress("Acción no permitida");return;}
 }
 else if(currentRole==="tecnico"){
  if(decision==="enviar_validacion"){
   p.step=7;p.state="Pendiente validación especializada";p.owner="Validador especializado";p.returned=false;p.rejected=false;action="Completó la evaluación técnica y la envió a validación especializada";
  }else if(decision==="guardar_borrador"){
   p.step=7;p.state="Evaluación técnica en curso";p.owner="Técnico evaluador";p.returned=false;p.rejected=false;action="Guardó avances de la evaluación técnica";
  }else{failHubOperationProgress("Acción no permitida");alert("El Técnico evaluador diligencia información y la envía a validación. No aprueba su propia evaluación.");return;}
 }
 else if(currentRole==="validador"){
  if(decision==="aprobar"){
   p.step=8;p.state="Evaluación técnica validada";p.owner="Secretaría Técnica / SPS";p.returned=false;p.rejected=false;action="Validó la evaluación técnica y la remitió a Secretaría Técnica / SPS";
  }else if(decision==="devolver"){
   p.step=7;p.state="Devuelto al técnico para ajustes";p.owner="Técnico evaluador";p.returned=true;p.rejected=false;action="Devolvió la evaluación al Técnico evaluador con observaciones";
  }else{failHubOperationProgress("Acción no permitida");return;}
 }
 else if(currentRole==="decision"){
  if(decision==="priorizar"){
   p.step=10;p.state="Priorizada";p.owner="Proponente / Formulador";p.returned=false;p.rejected=false;action="Priorizó la iniciativa y habilitó su continuación";
  }else if(decision==="aclaracion"){
   p.step=8;p.state="Aclaración solicitada";p.owner="Secretaría Técnica / SPS";p.returned=true;p.rejected=false;action="Solicitó aclaración antes de tomar la decisión";
  }else if(decision==="banco"){
   p.state="Banco de iniciativas";p.owner="Secretaría Técnica / SPS";p.returned=false;p.rejected=false;action="Mantuvo la iniciativa en el banco de información";
  }else{failHubOperationProgress("Acción no permitida");return;}
 }
 else{
  failHubOperationProgress("Perfil sin acción operativa");alert("Este perfil no participa en la cadena de aprobación de la iniciativa.");return;
 }
 ...
 try{
  ...
  await writeTrace(p,"GESTION_FLUJO",p.id,{descripcion:action,comentario:comment,pasoOrigen,estadoOrigen,pasoDestino:p.step,estadoDestino:p.state,fechaGestion:gestionFecha,recibidoPor:steps.find(s=>s.n==pasoOrigen)?.role||"",entregadoA:p.owner||steps.find(s=>s.n==p.step)?.role||""});
  setHubOperationProgress(60,"Actualizando el flujo compartido y la bandeja siguiente…");
  await saveSharedWorkflow(p,{date:gestionFecha,action,comment,stepFrom:pasoOrigen,stateFrom:estadoOrigen,stepTo:p.step,stateTo:p.state,ownerTo:p.owner,updatedBy:actorLabel()});
  setHubOperationProgress(82,"Actualizando el reporte de gestión…");
  await writeManagementHistoryDocument(p);
 }catch(e){
  console.warn("No se pudo persistir completamente el movimiento compartido",e);
  failHubOperationProgress("La gestión cambió en pantalla, pero falló la sincronización");
  setTimeout(()=>alert("Hubo un problema al sincronizar la gestión con OneDrive: "+e.message),400);
  return;
 }
...
async function addProject(){
 ...
 try{
  ...
  await saveProjectGeometry(id,{...pendingProjectGeometry,source:"user"});pendingProjectGeometry=null;originalProjectGeometry=null;projectGeometryDirty=false;

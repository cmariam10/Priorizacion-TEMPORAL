...
function buildProjectApiPayload(){
 ...
 for(const h of excelHeaders){
  if(hnorm(h)===hnorm("ID HUB")||
     hnorm(h)===hnorm("Fecha actualización")||
     hnorm(h)===hnorm("Estado HUB")||
     hnorm(h)===hnorm("Referencia de fila"))continue;
...
async function addProject(){
 ...
 try{
  ...
  const registration=await hubApiFetch("/projects",{
   method:"POST",
   headers:{"Content-Type":"application/json"},
   body:JSON.stringify(buildProjectApiPayload())
  });
  const id=String(registration?.idHub||"").trim();
  if(!id)throw new Error("El Worker registró la iniciativa, pero no devolvió el ID HUB.");

  // La ubicación también se guarda mediante el Worker. La cuenta Microsoft
  // del usuario nunca accede directamente al OneDrive central.
  await hubApiFetch(`/projects/${encodeURIComponent(id)}/geometry`,{
   method:"PUT",
   headers:{"Content-Type":"application/json"},
   body:JSON.stringify({...pendingProjectGeometry,source:"user"})
  });

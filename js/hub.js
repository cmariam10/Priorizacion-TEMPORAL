...
async function resolveSharedDriveItem(shareUrl){
 ...
 // Resolve the share itself first. This avoids relying on the /driveItem
 // relationship redirect, which can return HTTP 308 for cross-tenant users.
 const shared=await graphFetch(`/shares/${token}?$expand=driveItem`,{
  headers:{"Prefer":"redeemSharingLink"}
 });
 if(shared?.driveItem)return shared.driveItem;

 // Fallback for tenants that do not expand the relationship in the first call.
 // Once the sharing gesture has been redeemed, this normally resolves directly.
 return await graphFetch(`/shares/${token}/driveItem`,{

...
function dbOpen(){
 return new Promise((resolve,reject)=>{const req=indexedDB.open("hub_documentos_db",1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains("files"))db.createObjectStore("files")};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});
}
async function storeFile(key,file){const db=await dbOpen();return new Promise((resolve,reject)=>{const tx=db.transaction("files","readwrite");tx.objectStore("files").put(file,key);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);});}
async function openStoredFile(key,name){
 try{const db=await dbOpen();const file=await new Promise((resolve,reject)=>{const tx=db.transaction("files","readonly");const r=tx.objectStore("files").get(key);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});if(!file){alert("El archivo no está disponible en este navegador.");return;}const url=URL.createObjectURL(file);const a=document.createElement("a");a.href=url;a.target="_blank";a.rel="noopener";a.download=name||file.name||"documento";a.click();setTimeout(()=>URL.revokeObjectURL(url),3000);}catch(e){alert("No fue posible abrir el archivo guardado.");}
}
const roleInfo={
...
function simulateDocUpload(id){
 ...
 input.onchange=async()=>{
  ...
  const key=`${currentUser?.username||"anon"}_${id}_${Date.now()}_${file.name}`;
  try{await storeFile(key,file);}catch(e){alert("No fue posible guardar el archivo en este navegador.");return;}
  documents.unshift({project:id,type:"Soporte cargado",name:file.name,state:"Borrador",owner:actorLabel(),fileKey:key,size:file.size,uploadedAt:new Date().toLocaleString()});
  auditLog.unshift({project:id,action:"Cargó soporte documental",role:actorLabel(),date:new Date().toLocaleString(),comment:`Se cargó ${file.name}.`});
  saveHubState();renderDocuments();openManageModal(id);alert("Documento guardado. Permanecerá visible para esta persona de prueba aunque cambie de perfil o cierre y vuelva a iniciar sesión en este navegador.");
 };input.click();
}

function docViewAction(d){return d.fileKey?`openStoredFile(${JSON.stringify(d.fileKey)},${JSON.stringify(d.name)})`:`alert(\'Documento de demostración sin archivo físico asociado.\')`;}
...
function addProject(){
 ...
 auditLog.unshift({project:id,action:"Registró iniciativa",role:actorLabel(),date:new Date().toLocaleString(),comment:"Registro inicial del proyecto."}); saveHubState();
 populateSelects(); renderKpis(); renderMatrix(); renderList(); selectProject(id); alert("Iniciativa registrada en el paso 1. Se notificó a la jefatura técnica para revisión.");
}

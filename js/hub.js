...
function renderRegistrationForm(data={}){
 ...
 setTimeout(()=>initRegistrationLocation(window.__editingGeometry||null),120);
}
...
async function editProjectExcel(id){
 try{
  ...
  // Mostrar primero el formulario para que el usuario vea que la edición se está cargando.
  const data={...(p.excelData||{})};
  window.__editingGeometry=null;
  ...
  if(status)status.innerHTML="<b>Cargando ubicación guardada…</b>";

  // Recuperar siempre la geometría persistida del proyecto desde el HUB API.
  let g=await loadProjectGeometry(id);
  if(!g||!validProjectGeometry(g)){
   const x=inferGeo(p);
   g={type:"point",source:"inferred",referencePoints:[{lat:Number(x.lat),lon:Number(x.lon)}],inferredFrom:x.fuente};
  }
  window.__editingGeometry=g;

  // Inicializar nuevamente el mapa ya visible con la geometría recuperada.
...
async function addProject(){
 const btn=document.getElementById("saveExcelProjectBtn");
 const originalButtonText=btn?.textContent||"Guardar iniciativa";
 ...
 try{
  ...
  if(btn){
   ...
   btn.textContent=editId?"Guardando cambios…":"Guardando iniciativa…";
  ...
  if(editId){
   ...
   if(btn){delete btn.dataset.editId;}
   await loadProjectsFromExcel();

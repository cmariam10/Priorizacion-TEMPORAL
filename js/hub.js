...
async function focusProjectFromList(id){
 ...
 if(g?.type==="segment"&&g.coordinates?.length>1){
  portfolioSearchFocusLayer=L.polyline(
   g.coordinates.map(c=>[Number(c[1]),Number(c[0])]),
   {weight:9,opacity:1}
    ...
    <tbody>${list.slice(0,400).map(p=>{
      ...
      return `<tr>
        ...
        <td class="pl-actions"><div class="project-action-grid">
          ...
          <button onclick="renderFicha(\'${p.id}\')">Ficha</button><button onclick="viewProjectDocuments('${p.id}')">Documentos</button>

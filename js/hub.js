...
window.HUB_ONEDRIVE_BUILD = "2026-09-21-online-edit-v15";
...
async function replaceOneDriveDocument(index){
 ...
 input.onchange=async()=>{
  ...
  try{
   showHubOperationProgress("Cargando nueva versión",`Preparando ${d.name}…`,15);
   const file=selected.name===d.name?selected:new File([selected],d.name,{type:selected.type||"application/octet-stream",lastModified:selected.lastModified});
   const form=new FormData(); form.append("file",file,d.name);
   setHubOperationProgress(45,"Enviando la nueva versión al repositorio central…");
   await hubApiFetch(`/projects/${encodeURIComponent(p.id)}/documents/replace`,{method:"PUT",body:form});
   setHubOperationProgress(82,"Actualizando el listado de documentos…");
   await syncProjectDocuments(p.id);
   finishHubOperationProgress("Nueva versión guardada correctamente");
  }catch(err){console.error(err);failHubOperationProgress("No fue posible actualizar el documento");setTimeout(()=>alert("No fue posible actualizar el documento: "+(err.message||err)),350);}
 }; input.click();
}

...
async function resolveExcelWorkbook(){
 ...
 // Resolver siempre desde el vínculo compartido. Los driveId/itemId de SharePoint
 // pueden cambiar y un identificador guardado previamente puede devolver ObjectHandle is Invalid.
 try{
 ...
 // Compatibilidad con instalaciones donde los IDs fijos aún sean válidos.
...
async function resolveRootFolder(){
 ...
 // Resolver la carpeta raíz desde el vínculo compartido para no depender de
 // driveId/itemId persistidos que pueden quedar obsoletos en SharePoint/OneDrive.
 try{
...
async function loginRole(){
 ...
 try{
   ...
   try{
     ...
     await loadAllSharedWorkflow();
     startSharedFlowSync();
   }catch(excelErr){console.error(excelErr);alert("Sesión iniciada, pero no fue posible cargar/sincronizar la base maestra y el flujo compartido: "+excelErr.message);}

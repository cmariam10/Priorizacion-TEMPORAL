...
window.HUB_ONEDRIVE_BUILD = "2026-09-21-documents-worker-modal-layer";
...
async function syncProjectDocuments(id){
 ...
 const remote=(result.documents||[]).map(x=>({
  project:id,
  type:"Soporte cargado",
  name:x.name,
  state:"En OneDrive",
  owner:"Repositorio MOPT",
  itemId:x.id,
  webUrl:x.webUrl||"",
  size:Number(x.size||0),
  uploadedAt:x.lastModifiedDateTime||x.createdDateTime||"",
  mimeType:x.mimeType||"",
  eTag:x.eTag||""
 }));
...
async function openOneDriveDocument(d){
 try{
  ...
  alert("Este documento no tiene un enlace de apertura disponible.");
 }catch(e){alert(e.message||"No fue posible abrir el documento.");}
}

async function replaceOneDriveDocument(index){
 const d=documents[index],p=projects.find(x=>x.id===d?.project);
 if(!d||!p||!d.itemId){alert("Este documento no está vinculado a un archivo físico en OneDrive.");return;}
 ...
 input.onchange=async()=>{
  ...
  try{
   ...
   // El Worker conserva el mismo DriveItem buscando el nombre actual del archivo.
   const file=selected.name===d.name
    ? selected
    : new File([selected],d.name,{type:selected.type||"application/octet-stream",lastModified:selected.lastModified});
   const form=new FormData();
   form.append("file",file,d.name);
   setHubOperationProgress(45,"Enviando la nueva versión al repositorio central…");
   const result=await hubApiFetch(`/projects/${encodeURIComponent(p.id)}/documents/replace`,{method:"PUT",body:form});
   setHubOperationProgress(82,"Actualizando el listado de documentos…");
   await syncProjectDocuments(p.id);
   auditLog.unshift({project:p.id,action:"Actualizó / sobrescribió documento",role:actorLabel(),date:new Date().toISOString(),comment:`Se actualizó ${d.name} mediante el repositorio central.`});
   saveHubState();
   if(manageModal.classList.contains("open")&&activeId===p.id)openManageModal(p.id,true);
   finishHubOperationProgress("Nueva versión guardada correctamente");
  }catch(e){
   console.error(e);
   failHubOperationProgress("No fue posible actualizar el documento");
   setTimeout(()=>alert("No fue posible actualizar el documento: "+(e.message||e)),350);
  }
 };
 input.click();
}
...
async function deleteOneDriveDocument(index){
 const d=documents[index],p=projects.find(x=>x.id===d?.project);
 if(!d||!p||!d.itemId){alert("Este documento no tiene un archivo físico de OneDrive asociado.");return;}
 ...
 try{
  ...
  setHubOperationProgress(80,"Actualizando el listado…");
  await syncProjectDocuments(p.id);
  auditLog.unshift({project:p.id,action:"Eliminó documento",role:actorLabel(),date:new Date().toISOString(),comment:`Se eliminó ${d.name} mediante el repositorio central.`});
  saveHubState();
  if(manageModal.classList.contains("open")&&activeId===p.id)openManageModal(p.id,true);
  finishHubOperationProgress("Documento eliminado");
 }catch(e){
  console.error(e);
  failHubOperationProgress("No fue posible eliminar el documento");
  const msg=String(e?.message||e);
  setTimeout(()=>alert(msg.includes("423")
   ?"No se puede eliminar el documento porque está abierto o bloqueado en Microsoft 365. Cierre el archivo y vuelva a intentarlo."
   :"No fue posible eliminar el documento: "+msg),350);
 }
}
...
function elevateManageModal(){
 if(!manageModal)return;
 if(manageModal.parentElement!==document.body)document.body.appendChild(manageModal);
 manageModal.style.position="fixed";
 manageModal.style.inset="0";
 manageModal.style.zIndex="2147483000";
 manageModal.style.isolation="isolate";
 const card=manageModal.querySelector(".modal-card,.modal-content,.manage-modal-card");
 if(card){
...
(function injectProjectListStyles(){
  ...
  s.textContent=`
    ...
    #manageModal.open,#manageModal{z-index:2147483000!important}
    #manageModal .modal-card,#manageModal .modal-content{position:relative!important;z-index:2147483001!important}
    #manageModal .leaflet-container{z-index:1!important}
...
async function simulateDocUpload(id){
 ...
 input.onchange=async()=>{
  ...
  try{
   ...
   setHubOperationProgress(40,"Enviando el archivo al repositorio central…");
   ...
   await syncProjectDocuments(id);
   auditLog.unshift({project:id,action:"Cargó soporte documental",role:actorLabel(),date:new Date().toISOString(),comment:`Se cargó ${file.name} mediante el repositorio central.`});
   saveHubState();
   ...
   console.error(e);
   failHubOperationProgress("No fue posible cargar el documento");
   setTimeout(()=>alert("No fue posible guardar el archivo: "+(e.message||e)),350);
  }
 };
 input.click();
}
...
function setProjectSaveBusy(btn,busy,editing=false){
 let overlay=document.getElementById("hubProjectSavingOverlay");
 ...
 if(busy){
  ...
  if(!overlay){
   ...
   overlay.id="hubProjectSavingOverlay";
   overlay.style.cssText="position:fixed;inset:0;z-index:999999;background:rgba(10,28,52,.34);display:flex;align-items:center;justify-content:center;padding:20px;";
   overlay.innerHTML=`<div style="background:#fff;border-radius:16px;box-shadow:0 18px 55px rgba(0,0,0,.24);padding:24px 30px;min-width:280px;max-width:90vw;text-align:center;font-family:inherit">
     <div style="font-size:20px;font-weight:800;color:#0b3f75;margin-bottom:8px">${editing?"Guardando cambios…":"Guardando iniciativa…"}</div>
     <div style="color:#5d6b7d;font-size:14px">Actualizando la información. Por favor espere.</div>
     <div style="margin:18px auto 0;width:34px;height:34px;border:4px solid #d8e4f1;border-top-color:#0b4f8a;border-radius:50%;animation:hubSavingSpin .8s linear infinite"></div>
    </div>`;
   if(!document.getElementById("hubSavingSpinStyle")){
    const st=document.createElement("style");
    st.id="hubSavingSpinStyle";
    st.textContent="@keyframes hubSavingSpin{to{transform:rotate(360deg)}}";
   ...
   if(title)title.textContent=editing?"Guardando cambios…":"Guardando iniciativa…";
  }
 }else{
  if(btn){
   btn.disabled=false;

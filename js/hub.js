...
const roleStepPermissions={tecnico:[],jefatura:[2],planeamiento:[3],direccion:[4],coordinador:[5],secretaria:[6,8],areas:[7],sistema:[8],comite:[9],ejecutor:[10,11,12,13,14,15,16,17],administrador:Array.from({length:17},(_,i)=>i+1)};
let auditLog=[];
...
const ROLE_DISPLAY_STEPS={tecnico:"1",jefatura:"2",planeamiento:"3",direccion:"4",coordinador:"5",secretaria:"6 y 8",areas:"7",sistema:"8",comite:"9",ejecutor:"10–17",administrador:"Gobernanza"};
...
function notificationProjects(){
 ...
 if(currentRole==="administrador")return projects.filter(p=>p.sharedUpdatedAt||p.governance);
...
async function refreshBadgesAfterSharedSync(){
 ...
 try{
  ...
  // El flujo central puede cambiar paso, estado y responsable.
  // Redibujar todas las vistas que dependen de esos valores.
  renderFlow();renderKpis();renderMatrix();renderDetail();renderList();renderInbox();if(currentRole==="administrador")renderAdminGovernance();
  await renderNotifications();
 }catch(e){console.warn("No se pudo refrescar el flujo para las vistas",e);}
 refreshWorkBadges();
}
...
async function resolveSharedFlowFolder(){return {worker:true};}
function workflowPayload(p){return {projectId:p.id,step:Number(p.step)||1,state:p.state||"",owner:p.owner||"",returned:!!p.returned,rejected:!!p.rejected,updatedAt:new Date().toISOString(),updatedBy:actorLabel()};}
async function saveSharedWorkflow(p,movement=null){
 const result=await hubApiFetch(`/projects/${encodeURIComponent(p.id)}/workflow`,{method:"PUT",body:JSON.stringify({...workflowPayload(p),movement:movement||null})});
 return result?.workflow||workflowPayload(p);
}
function applySharedWorkflow(p,s){
 if(!s)return;p.step=Number(s.step)||p.step||1;p.state=s.state||p.state;p.owner=s.owner||p.owner;p.returned=!!s.returned;p.rejected=!!s.rejected;p.sharedUpdatedAt=s.updatedAt||null;p.sharedUpdatedBy=s.updatedBy||null;
}
async function loadAllSharedWorkflow(){
 try{
  const result=await hubApiFetch("/workflow"),byId=new Map();
  for(const s of result?.workflows||[])if(s?.projectId)byId.set(String(s.projectId),s);
  projects.forEach(p=>applySharedWorkflow(p,byId.get(String(p.id))));
  return byId;
 }catch(e){console.warn("No se pudo sincronizar el flujo compartido desde el Worker",e);return new Map();}
}
async function recoverLegacyWorkflowFromTrace(){
 // Recuperación histórica desactivada en navegador para no usar Graph delegado.
 return;
}
...
let hubApiPending=0,hubApiLoadingTimer=null;
function showApiActivity(){hubApiPending++;if(hubApiPending===1){clearTimeout(hubApiLoadingTimer);hubApiLoadingTimer=setTimeout(()=>{if(hubApiPending>0&&!document.getElementById("hubOperationProgress")?.classList.contains("show")){let n=document.getElementById("hubApiActivity");if(!n){n=document.createElement("div");n.id="hubApiActivity";n.setAttribute("role","status");n.setAttribute("aria-live","polite");n.innerHTML='<span class="api-spinner"></span><span>Cargando…</span>';document.body.appendChild(n);}n.classList.add("show");}},350);}}
function hideApiActivity(){hubApiPending=Math.max(0,hubApiPending-1);if(!hubApiPending){clearTimeout(hubApiLoadingTimer);document.getElementById("hubApiActivity")?.classList.remove("show");}}
...
// ===== Gobernanza central V25: plazos por paso + Administrador HUB =====
const GOVERNANCE_DEFAULTS={maxBusinessDays:180,warningBusinessDays:30,updateRequestBusinessDays:5};
const DEFAULT_STEP_DEADLINES={1:5,2:5,3:5,4:5,5:3,6:5,7:10,8:2,9:10,10:30,11:5,12:5,13:15,14:60,15:10,16:10,17:15};
let governanceConfig={stepDeadlines:{...DEFAULT_STEP_DEADLINES},maxBusinessDays:180,warningBusinessDays:30,updateRequestBusinessDays:5,warningBeforeDays:3};
function isGovernanceAdmin(){return currentRole==='administrador';}
function parseHubDate(value){
 if(value===null||value===undefined||value==='')return null;
 if(value instanceof Date)return Number.isFinite(value.getTime())?value:null;
 if(typeof value==='number'||(/^\d+(?:\.\d+)?$/.test(String(value).trim())&&Number(value)>20000)){const n=Number(value);if(Number.isFinite(n)){const d=new Date(Date.UTC(1899,11,30)+Math.round(n*86400000));return Number.isFinite(d.getTime())?d:null;}}
 const s=String(value).trim();
 let m=s.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?/);
 if(m){const d=new Date(Number(m[3]),Number(m[2])-1,Number(m[1]),Number(m[4]||12),Number(m[5]||0),Number(m[6]||0));return Number.isFinite(d.getTime())?d:null;}
 const d=new Date(s);return Number.isFinite(d.getTime())?d:null;
}
function businessDaysBetween(a,b){let d=parseHubDate(a),end=parseHubDate(b),n=0;if(!d||!end)return null;d=new Date(d);end=new Date(end);d.setHours(12,0,0,0);end.setHours(12,0,0,0);const sign=d<=end?1:-1;if(sign<0){const tmp=d;d=end;end=tmp;}while(d<end){d.setDate(d.getDate()+1);const w=d.getDay();if(w!==0&&w!==6)n++;}return n*sign;}
function addBusinessDays(date,days){let d=parseHubDate(date)||new Date();d=new Date(d);let n=Math.max(0,Number(days)||0);while(n>0){d.setDate(d.getDate()+1);if(d.getDay()!==0&&d.getDay()!==6)n--;}return d.toISOString();}
function excelUpdatedAt(p){const d=p?.excelData||{};for(const [k,v] of Object.entries(d)){if(hnorm(k)===hnorm('Fecha actualización')&&v!==null&&v!==undefined&&String(v).trim()!=='')return v;}return '';}
function deriveFreshness(p){const last=excelUpdatedAt(p)||p.sharedUpdatedAt||'';const days=businessDaysBetween(last,new Date());if(days===null)return {code:'unknown',label:'Sin fecha de actualización',days:null,last};const max=Number(p.governance?.maxBusinessDays||governanceConfig.maxBusinessDays||GOVERNANCE_DEFAULTS.maxBusinessDays),warn=Number(p.governance?.warningBusinessDays||governanceConfig.warningBusinessDays||GOVERNANCE_DEFAULTS.warningBusinessDays);if(p.governance?.updateRequest?.status==='Pendiente')return {code:'requested',label:'Actualización solicitada',days,last};if(days>max)return {code:'update_required',label:'Requiere actualización',days,last};if(days>=Math.max(0,max-warn))return {code:'warning',label:'Próximo a actualizar',days,last};return {code:'current',label:'Vigente',days,last};}
function governanceBadge(p){const f=deriveFreshness(p);const icon=f.code==='update_required'?'🔴':f.code==='requested'?'🔵':f.code==='warning'?'🟡':f.code==='current'?'🟢':'⚪';return `${icon} ${f.label}`;}
function applyGovernance(p,g){p.governance=g||{};if(g?.lifecycleState)p.lifecycleState=g.lifecycleState;if(g?.assignment)p.assignment=g.assignment;else p.assignment=null;}
async function loadGovernanceConfig(){try{const r=await hubApiFetch('/governance/config');governanceConfig={...governanceConfig,...(r?.config||{}),stepDeadlines:{...DEFAULT_STEP_DEADLINES,...(r?.config?.stepDeadlines||{})}};}catch(e){console.warn('No se pudo cargar parametrización de gobernanza',e);}return governanceConfig;}
async function loadAllGovernance(){try{await loadGovernanceConfig();const r=await hubApiFetch('/governance'),m=new Map();for(const g of r?.governance||[])if(g?.projectId)m.set(String(g.projectId),g);projects.forEach(p=>applyGovernance(p,m.get(String(p.id))||null));return m;}catch(e){console.warn('No se pudo sincronizar gobernanza',e);return new Map();}}
async function saveProjectGovernance(patch,id){const p=projects.find(x=>x.id===id);if(!p)throw new Error('Proyecto no encontrado.');const body={projectId:id,lifecycleState:p.lifecycleState||p.governance?.lifecycleState||'',lastValidatedAt:p.governance?.lastValidatedAt||excelUpdatedAt(p)||new Date().toISOString(),maxBusinessDays:Number(p.governance?.maxBusinessDays||governanceConfig.maxBusinessDays),warningBusinessDays:Number(p.governance?.warningBusinessDays||governanceConfig.warningBusinessDays),assignment:p.assignment||p.governance?.assignment||null,...patch,updatedBy:actorLabel()};const r=await hubApiFetch(`/projects/${encodeURIComponent(id)}/governance`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});applyGovernance(p,r?.governance||body);return r?.governance||body;}
function stepDeadline(step){return Math.max(1,Number(governanceConfig.stepDeadlines?.[Number(step)]||DEFAULT_STEP_DEADLINES[Number(step)]||10));}
function buildStepAssignment(p,observations=''){const st=steps.find(s=>s.n===Number(p.step));const now=new Date().toISOString(),days=stepDeadline(p.step);return {activity:st?.title||'Gestión del proyecto',step:Number(p.step),responsibleRole:st?.role||p.owner||'',responsible:p.owner||st?.role||'',institution:p.inst||'',assignedAt:now,businessDays:days,originalDueAt:addBusinessDays(now,days),dueAt:addBusinessDays(now,days),status:'Asignada',lastUpdatedAt:now,observations,extension:null};}
async function confirmProjectUpdate(id){try{const p=projects.find(x=>x.id===id);const req=p?.governance?.updateRequest;await saveProjectGovernance({lastValidatedAt:new Date().toISOString(),updateRequest:req?{...req,status:'Completada',completedAt:new Date().toISOString(),completedBy:actorLabel()}:null},id);await loadProjectsFromExcel();alert(`${id} quedó marcado como actualizado y vigente.`);}catch(e){alert('No fue posible actualizar la vigencia: '+e.message);}}
async function requestProjectUpdate(id){if(!isGovernanceAdmin())return alert('Solo el Administrador HUB puede solicitar formalmente la actualización.');const p=projects.find(x=>x.id===id);if(!p)return;const reason=prompt('Motivo de la solicitud de actualización:','Revisión periódica de vigencia de la información.');if(reason===null)return;const now=new Date().toISOString(),days=Number(governanceConfig.updateRequestBusinessDays||5);const updateRequest={status:'Pendiente',requestedAt:now,requestedBy:actorLabel(),reason:reason.trim(),businessDays:days,dueAt:addBusinessDays(now,days),responsible:p.owner||steps.find(s=>s.n===Number(p.step))?.role||''};try{await saveProjectGovernance({updateRequest},id);renderInbox();await renderNotifications();renderAdminGovernance();openManageModal(id,true);alert(`Solicitud de actualización creada. Plazo automático: ${days} días hábiles.`);}catch(e){alert('No fue posible solicitar la actualización: '+e.message);}}
async function markProjectExecuted(id){if(!isGovernanceAdmin()&&currentRole!=='ejecutor')return alert('Este perfil no puede marcar proyectos como ejecutados.');const p=projects.find(x=>x.id===id);if(!p)return;const note=prompt('Observación de finalización de la ejecución (opcional):','')??null;if(note===null)return;try{await saveProjectGovernance({lifecycleState:'Ejecutado',executedAt:new Date().toISOString(),executionNote:note,assignment:null},id);p.state='Ejecutado';p.lifecycleState='Ejecutado';p.assignment=null;renderKpis();renderList();renderInbox();await renderNotifications();openManageModal(id,true);alert(`${id} quedó marcado como Ejecutado y sincronizado para todos.`);}catch(e){alert('No fue posible marcar el proyecto como ejecutado: '+e.message);}}
async function requestExtension(id){const p=projects.find(x=>x.id===id),a=p?.assignment;if(!a)return alert('Este proyecto no tiene una asignación activa.');const extra=Number(prompt('Días hábiles adicionales solicitados:','5'));if(!Number.isFinite(extra)||extra<1)return;const justification=prompt('Justificación de la prórroga:','');if(justification===null||!justification.trim())return alert('Debe registrar una justificación.');const extension={status:'Solicitada',requestedAt:new Date().toISOString(),requestedBy:actorLabel(),justification:justification.trim(),extraBusinessDays:extra,originalDueAt:a.originalDueAt||a.dueAt};try{await saveProjectGovernance({assignment:{...a,extension,lastUpdatedAt:new Date().toISOString()}},id);renderInbox();await renderNotifications();openManageModal(id,true);alert('Prórroga enviada al Administrador HUB para decisión.');}catch(e){alert('No fue posible solicitar la prórroga: '+e.message);}}
async function decideExtension(id,approve){if(!isGovernanceAdmin())return alert('Solo el Administrador HUB puede decidir prórrogas.');const p=projects.find(x=>x.id===id),a=p?.assignment,ex=a?.extension;if(!ex||ex.status!=='Solicitada')return alert('No hay una solicitud de prórroga pendiente.');const now=new Date().toISOString();let next={...ex,status:approve?'Aprobada':'Rechazada',decidedAt:now,decidedBy:actorLabel()};let nextA={...a,extension:next,lastUpdatedAt:now};if(approve){nextA.dueAt=addBusinessDays(a.dueAt,Number(ex.extraBusinessDays||0));nextA.status='Prorrogada';next.newDueAt=nextA.dueAt;}try{await saveProjectGovernance({assignment:nextA},id);renderInbox();await renderNotifications();renderAdminGovernance();openManageModal(id,true);alert(approve?'Prórroga aprobada y nueva fecha calculada.':'Prórroga rechazada. Se conserva el vencimiento vigente.');}catch(e){alert('No fue posible registrar la decisión: '+e.message);}}
async function configureGovernanceDeadlines(){if(!isGovernanceAdmin())return;let lines=steps.map(s=>`${s.n}=${stepDeadline(s.n)}`).join('\n');const raw=prompt('Plazos estándar por paso (días hábiles). Formato: paso=días, una línea por paso.',lines);if(raw===null)return;const parsed={...governanceConfig.stepDeadlines};for(const line of raw.split(/\n+/)){const m=line.trim().match(/^(\d{1,2})\s*=\s*(\d{1,3})$/);if(m&&Number(m[1])>=1&&Number(m[1])<=17&&Number(m[2])>=1)parsed[Number(m[1])]=Number(m[2]);}try{const r=await hubApiFetch('/governance/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...governanceConfig,stepDeadlines:parsed,updatedBy:actorLabel()})});governanceConfig={...governanceConfig,...r.config,stepDeadlines:{...parsed,...(r.config?.stepDeadlines||{})}};renderInbox();alert('Plazos estándar actualizados. Aplicarán a los proyectos que entren posteriormente a cada paso.');}catch(e){alert('No fue posible guardar la parametrización: '+e.message);}}
window.confirmProjectUpdate=confirmProjectUpdate;window.requestProjectUpdate=requestProjectUpdate;window.markProjectExecuted=markProjectExecuted;window.requestExtension=requestExtension;window.decideExtension=decideExtension;window.configureGovernanceDeadlines=configureGovernanceDeadlines;
async function adminStartCurrentDeadline(id){
 if(!isGovernanceAdmin())return alert('Solo el Administrador HUB puede iniciar el seguimiento temporal.');
 const p=projects.find(x=>x.id===id);if(!p)return;
 if(p.assignment?.dueAt)return alert('El proyecto ya tiene un plazo activo.');
 const st=steps.find(s=>s.n===Number(p.step)),days=stepDeadline(p.step);
 if(!confirm(`Se iniciará hoy el plazo del paso ${p.step}. ${st?.title||''} con ${days} días hábiles.\n\nUse esta acción solo cuando corresponda formalmente iniciar el seguimiento del proyecto histórico.`))return;
 const reason=prompt('Observación / fundamento para iniciar el plazo:','Inicio de seguimiento temporal del paso actual por Administrador HUB.');if(reason===null)return;
 const assignment=buildStepAssignment(p,reason.trim());
 assignment.startedBy=actorLabel();assignment.startSource='Administrador HUB';
 try{await saveProjectGovernance({assignment},id);p.assignment=assignment;renderAdminGovernance();await renderNotifications();alert(`Plazo iniciado: ${days} días hábiles. Vence ${adminFmtDate(assignment.dueAt)}.`);}catch(e){alert('No fue posible iniciar el plazo: '+e.message);}
}
async function adminAdjustActiveDeadline(id){
 if(!isGovernanceAdmin())return alert('Solo el Administrador HUB puede ajustar un plazo activo.');
 const p=projects.find(x=>x.id===id),a=p?.assignment;if(!a?.dueAt)return alert('El proyecto no tiene un plazo activo.');
 const raw=prompt('Nuevo plazo total del paso, en días hábiles:',String(a.businessDays||stepDeadline(p.step)));if(raw===null)return;
 const n=Number(raw);if(!Number.isInteger(n)||n<1||n>365)return alert('Ingrese un número entero entre 1 y 365.');
 const reason=prompt('Justificación obligatoria del ajuste de plazo:','');if(reason===null||!reason.trim())return alert('Debe registrar una justificación.');
 const now=new Date().toISOString(),base=a.assignedAt||now,newDue=addBusinessDays(base,n),history=Array.isArray(a.deadlineHistory)?[...a.deadlineHistory]:[];
 history.push({changedAt:now,changedBy:actorLabel(),previousBusinessDays:Number(a.businessDays||0),previousDueAt:a.dueAt||'',newBusinessDays:n,newDueAt:newDue,reason:reason.trim()});
 const next={...a,businessDays:n,dueAt:newDue,status:'En gestión',lastUpdatedAt:now,deadlineHistory:history};
 try{await saveProjectGovernance({assignment:next},id);p.assignment=next;renderAdminGovernance();await renderNotifications();alert(`Plazo activo ajustado. Nuevo vencimiento: ${adminFmtDate(newDue)}. El vencimiento anterior quedó en el historial.`);}catch(e){alert('No fue posible ajustar el plazo: '+e.message);}
}
async function adminGrantExtension(id){
 if(!isGovernanceAdmin())return alert('Solo el Administrador HUB puede otorgar prórrogas.');
 const p=projects.find(x=>x.id===id),a=p?.assignment;if(!a?.dueAt)return alert('El proyecto no tiene un plazo activo.');
 const raw=prompt('Días hábiles adicionales de prórroga:','5');if(raw===null)return;const extra=Number(raw);if(!Number.isInteger(extra)||extra<1||extra>180)return alert('Ingrese entre 1 y 180 días hábiles.');
 const justification=prompt('Justificación obligatoria de la prórroga:','');if(justification===null||!justification.trim())return alert('Debe registrar una justificación.');
 const now=new Date().toISOString(),previousDue=a.dueAt,newDue=addBusinessDays(previousDue,extra),extensionHistory=Array.isArray(a.extensionHistory)?[...a.extensionHistory]:[];
 const grant={status:'Aprobada',requestedAt:now,requestedBy:actorLabel(),justification:justification.trim(),extraBusinessDays:extra,originalDueAt:previousDue,newDueAt:newDue,decidedAt:now,decidedBy:actorLabel(),directGrant:true};
 extensionHistory.push(grant);
 const next={...a,dueAt:newDue,status:'Prorrogada',lastUpdatedAt:now,extension:grant,extensionHistory};
 try{await saveProjectGovernance({assignment:next},id);p.assignment=next;renderAdminGovernance();await renderNotifications();alert(`Prórroga otorgada por ${extra} días hábiles. Nuevo vencimiento: ${adminFmtDate(newDue)}.`);}catch(e){alert('No fue posible otorgar la prórroga: '+e.message);}
}
window.adminStartCurrentDeadline=adminStartCurrentDeadline;window.adminAdjustActiveDeadline=adminAdjustActiveDeadline;window.adminGrantExtension=adminGrantExtension;


let adminGovernanceSort={field:'update',dir:'asc'};
function adminDateValue(v){const d=parseHubDate(v);return d?d.getTime():0;}
function adminDaysToDue(v){if(!v)return null;return businessDaysBetween(new Date(),v);}
function adminDeadlineState(p){const a=p.assignment;if(!a?.dueAt)return {code:'none',label:'Sin plazo'};const d=adminDaysToDue(a.dueAt);if(d<0)return {code:'overdue',label:`Vencido · ${Math.abs(d)} día(s)`};if(d<=Number(governanceConfig.warningBeforeDays||3))return {code:'warning',label:`Próximo · ${d} día(s)`};return {code:'ok',label:`En plazo · ${d} día(s)`};}
function adminEsc(s){return traceEscape(String(s??''));}
function adminFmtDate(v){const d=parseHubDate(v);return d?d.toLocaleDateString():'—';}
function adminSetSort(field){if(adminGovernanceSort.field===field)adminGovernanceSort.dir=adminGovernanceSort.dir==='asc'?'desc':'asc';else adminGovernanceSort={field,dir:'asc'};renderAdminGovernance();}
function adminSortIcon(field){return adminGovernanceSort.field===field?(adminGovernanceSort.dir==='asc'?' ↑':' ↓'):'';}
function adminProjectLastUpdate(p){return excelUpdatedAt(p)||p.sharedUpdatedAt||'';}
function adminFilteredProjects(){
 const q=(document.getElementById('adminSearch')?.value||'').trim().toLowerCase(),fresh=document.getElementById('adminFreshFilter')?.value||'',deadline=document.getElementById('adminDeadlineFilter')?.value||'',inst=document.getElementById('adminInstFilter')?.value||'';
 let arr=projects.filter(p=>String(p.id)!=='M-20-001').filter(p=>!q||[p.id,p.name,p.inst,p.owner,steps.find(s=>s.n==p.step)?.title].some(x=>String(x||'').toLowerCase().includes(q))).filter(p=>!fresh||deriveFreshness(p).code===fresh).filter(p=>!deadline||adminDeadlineState(p).code===deadline).filter(p=>!inst||p.inst===inst);
 const f=adminGovernanceSort.field,sgn=adminGovernanceSort.dir==='asc'?1:-1;
 arr.sort((a,b)=>{let av,bv;if(f==='update'){av=adminDateValue(adminProjectLastUpdate(a));bv=adminDateValue(adminProjectLastUpdate(b));}else if(f==='due'){av=adminDateValue(a.assignment?.dueAt);bv=adminDateValue(b.assignment?.dueAt);}else if(f==='step'){av=Number(a.step||0);bv=Number(b.step||0);}else if(f==='id'){av=String(a.id);bv=String(b.id);}else{av=String(a.name||'');bv=String(b.name||'');}return (av<bv?-1:av>bv?1:0)*sgn;});return arr;
}
function renderAdminGovernance(){
 const box=document.getElementById('adminGovernanceBox');if(!box)return;if(!isGovernanceAdmin()){box.innerHTML='<div class="note">Este módulo es exclusivo del Administrador HUB / Gobernanza.</div>';return;}
 const all=projects.filter(p=>String(p.id)!=='M-20-001'),fresh=all.map(p=>deriveFreshness(p)),deadline=all.map(p=>adminDeadlineState(p)),pendingExt=all.filter(p=>p.assignment?.extension?.status==='Solicitada'),pendingUpd=all.filter(p=>p.governance?.updateRequest?.status==='Pendiente'),executed=all.filter(p=>p.lifecycleState==='Ejecutado');
 const insts=[...new Set(all.map(p=>p.inst).filter(Boolean))].sort();
 const existing={q:document.getElementById('adminSearch')?.value||'',fresh:document.getElementById('adminFreshFilter')?.value||'',deadline:document.getElementById('adminDeadlineFilter')?.value||'',inst:document.getElementById('adminInstFilter')?.value||''};
 box.innerHTML=`
 <div class="kpis" style="margin:18px 0"><div class="kpi"><small>Portafolio</small><strong>${all.length}</strong></div><div class="kpi"><small>Requieren actualización</small><strong>${fresh.filter(x=>x.code==='update_required').length}</strong></div><div class="kpi"><small>En plazo</small><strong>${deadline.filter(x=>x.code==='ok').length}</strong></div><div class="kpi"><small>Próximos a vencer</small><strong>${deadline.filter(x=>x.code==='warning').length}</strong></div><div class="kpi"><small>Pasos vencidos</small><strong>${deadline.filter(x=>x.code==='overdue').length}</strong></div><div class="kpi"><small>Sin plazo activo</small><strong>${deadline.filter(x=>x.code==='none').length}</strong></div><div class="kpi"><small>Prórrogas pendientes</small><strong>${pendingExt.length}</strong></div><div class="kpi"><small>Ejecutados</small><strong>${executed.length}</strong></div></div>
 <div class="manage-card" style="margin-bottom:18px"><div class="section-head"><div><h3>Plazos predeterminados del ciclo</h3><p class="muted">El plazo se asigna automáticamente al entrar a cada paso. Los cambios aplican hacia adelante.</p></div></div><div style="overflow-x:auto"><table><thead><tr><th>Paso</th><th>Actividad</th><th>Responsable del flujo</th><th>Días hábiles</th><th></th></tr></thead><tbody>${steps.map(s=>`<tr><td>${s.n}</td><td>${adminEsc(s.title)}</td><td>${adminEsc(s.role)}</td><td><b>${stepDeadline(s.n)}</b></td><td><button class="secondary" onclick="adminEditStepDeadline(${s.n})">Editar</button></td></tr>`).join('')}</tbody></table></div></div>
 <div class="manage-card" style="margin-bottom:18px"><h3>Prórrogas pendientes</h3>${pendingExt.length?pendingExt.map(p=>{const e=p.assignment.extension;return `<div class="note" style="margin-bottom:8px"><b>${adminEsc(p.id)} · ${adminEsc(p.name)}</b><br><span class="muted">${adminEsc(p.assignment.activity||'')} · Vencimiento original: ${adminFmtDate(e.originalDueAt||p.assignment.dueAt)} · Solicita +${Number(e.extraBusinessDays||0)} días hábiles</span><br>${adminEsc(e.justification||'')}<div class="action-row" style="margin-top:8px"><button class="btn-approve" onclick="decideExtension('${adminEsc(p.id)}',true)">Aprobar</button><button class="btn-reject" onclick="decideExtension('${adminEsc(p.id)}',false)">Rechazar</button></div></div>`}).join(''):'<div class="note">No hay solicitudes de prórroga pendientes.</div>'}</div>
 <div class="manage-card"><div class="section-head"><div><h3>Seguimiento global de proyectos</h3><p class="muted">Ordene por actualización o vencimiento y solicite actualización cuando corresponda.</p></div><div class="counter">${pendingUpd.length} actualización(es) solicitada(s)</div></div>
 <div class="filter-panel"><div><label>Buscar</label><input id="adminSearch" value="${adminEsc(existing.q)}" oninput="renderAdminGovernance()" placeholder="ID, proyecto, institución, responsable..."/></div><div><label>Vigencia</label><select id="adminFreshFilter" onchange="renderAdminGovernance()"><option value="">Todas</option><option value="current">Vigente</option><option value="warning">Próximo a actualizar</option><option value="update_required">Requiere actualización</option><option value="requested">Actualización solicitada</option></select></div><div><label>Plazo del paso</label><select id="adminDeadlineFilter" onchange="renderAdminGovernance()"><option value="">Todos</option><option value="ok">En plazo</option><option value="warning">Próximo a vencer</option><option value="overdue">Vencido</option><option value="none">Sin plazo</option></select></div><div><label>Institución</label><select id="adminInstFilter" onchange="renderAdminGovernance()"><option value="">Todas</option>${insts.map(x=>`<option>${adminEsc(x)}</option>`).join('')}</select></div></div>
 <div style="overflow-x:auto"><table><thead><tr><th><button class="secondary" onclick="adminSetSort('id')">ID${adminSortIcon('id')}</button></th><th>Proyecto / institución</th><th><button class="secondary" onclick="adminSetSort('step')">Paso${adminSortIcon('step')}</button></th><th><button class="secondary" onclick="adminSetSort('update')">Última actualización${adminSortIcon('update')}</button></th><th>Vigencia</th><th><button class="secondary" onclick="adminSetSort('due')">Vencimiento paso${adminSortIcon('due')}</button></th><th>Acciones</th></tr></thead><tbody id="adminProjectRows"></tbody></table></div></div>`;
 const f1=document.getElementById('adminFreshFilter'),f2=document.getElementById('adminDeadlineFilter'),f3=document.getElementById('adminInstFilter');if(f1)f1.value=existing.fresh;if(f2)f2.value=existing.deadline;if(f3)f3.value=existing.inst;
 renderAdminProjectRows();
}
function renderAdminProjectRows(){const body=document.getElementById('adminProjectRows');if(!body)return;const arr=adminFilteredProjects();body.innerHTML=arr.slice(0,500).map(p=>{const fr=deriveFreshness(p),dl=adminDeadlineState(p),last=adminProjectLastUpdate(p),req=p.governance?.updateRequest,a=p.assignment,ex=a?.extension;return `<tr><td><b>${adminEsc(p.id)}</b></td><td>${adminEsc(p.name)}<br><span class="muted">${adminEsc(p.inst||'')}</span></td><td>${Number(p.step||0)}. ${adminEsc(steps.find(s=>s.n==p.step)?.title||'')}<br><span class="muted">${a?.assignedAt?`Inicio: ${adminFmtDate(a.assignedAt)}`:'Sin inicio temporal'}</span></td><td>${adminFmtDate(last)}${fr.days!=null?`<br><span class="muted">${fr.days} días hábiles</span>`:''}</td><td>${governanceBadge(p)}${req?.status==='Pendiente'?`<br><span class="muted">Solicitud vence ${adminFmtDate(req.dueAt)}</span>`:''}</td><td>${a?.dueAt?`${adminFmtDate(a.dueAt)}<br><span class="muted">${adminEsc(dl.label)} · ${Number(a.businessDays||0)} días hábiles</span>${ex?.status==='Aprobada'?`<br><span class="muted">Prórroga +${Number(ex.extraBusinessDays||0)} días</span>`:''}`:'—<br><span class="muted">Sin plazo activo</span>'}</td><td><div class="action-row"><button class="secondary" onclick="openManageModal('${adminEsc(p.id)}')">Ver ficha</button><button onclick="requestProjectUpdate('${adminEsc(p.id)}')" ${req?.status==='Pendiente'?'disabled title="Ya existe una solicitud pendiente"':''}>Solicitar actualización</button>${!a?.dueAt?`<button class="secondary" onclick="adminStartCurrentDeadline('${adminEsc(p.id)}')">Iniciar plazo</button>`:`<button class="secondary" onclick="adminAdjustActiveDeadline('${adminEsc(p.id)}')">Editar plazo</button><button class="secondary" onclick="adminGrantExtension('${adminEsc(p.id)}')">Dar prórroga</button>`}${a?.extension?.status==='Solicitada'?`<button class="btn-approve" onclick="decideExtension('${adminEsc(p.id)}',true)">Aprobar prórroga</button><button class="btn-reject" onclick="decideExtension('${adminEsc(p.id)}',false)">Rechazar</button>`:''}</div></td></tr>`}).join('')||'<tr><td colspan="7">No hay proyectos que coincidan con los filtros.</td></tr>';}
async function adminEditStepDeadline(step){if(!isGovernanceAdmin())return;const s=steps.find(x=>x.n===Number(step));const current=stepDeadline(step),raw=prompt(`Nuevo plazo estándar para el paso ${step}: ${s?.title||''} (días hábiles)`,String(current));if(raw===null)return;const n=Number(raw);if(!Number.isInteger(n)||n<1||n>365)return alert('Ingrese un número entero entre 1 y 365.');const next={...governanceConfig,stepDeadlines:{...governanceConfig.stepDeadlines,[step]:n},updatedBy:actorLabel()};try{const r=await hubApiFetch('/governance/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(next)});governanceConfig={...governanceConfig,...r.config,stepDeadlines:{...governanceConfig.stepDeadlines,...r.config?.stepDeadlines}};renderAdminGovernance();alert(`Paso ${step}: plazo estándar actualizado a ${n} días hábiles. No modifica asignaciones ya iniciadas.`);}catch(e){alert('No fue posible actualizar el plazo: '+e.message);}}
window.renderAdminGovernance=renderAdminGovernance;window.renderAdminProjectRows=renderAdminProjectRows;window.adminSetSort=adminSetSort;window.adminEditStepDeadline=adminEditStepDeadline;window.adminStartCurrentDeadline=adminStartCurrentDeadline;window.adminAdjustActiveDeadline=adminAdjustActiveDeadline;window.adminGrantExtension=adminGrantExtension;
...
async function getPortfolioGeometry(p){
 ...
 try{
  const persisted=await loadProjectGeometry(p.id);
  if(persisted&&validProjectGeometry(persisted)){portfolioMapGeometries.set(p.id,persisted);return persisted;}
 }catch(e){console.warn(`No se pudo cargar geometría central de ${p.id}`,e);}
...
async function initRegistrationLocation(existing=null){
 ...
 try{
  await loadLeaflet();
  for(let i=0;i<12 && (el.clientWidth<100 || el.clientHeight<100);i++){await new Promise(r=>setTimeout(r,80));}
  if(registrationMap){try{registrationMap.remove()}catch(e){}}
  ...
  redrawRegistrationGeometry();
  [0,120,350].forEach(ms=>setTimeout(()=>{try{registrationMap?.invalidateSize(true);if(pendingProjectGeometry)redrawRegistrationGeometry();}catch(e){}},ms));
...
async function loadProjectsFromExcel(){
 ...
 }).filter(p=>p.id && hnorm(p.excelData?.["Estado HUB"]||p.excelData?.estadoHub||"")!==hnorm("Eliminado"));
 activeId=projects[0]?.id||"";
 await loadAllSharedWorkflow();
 await loadAllGovernance();
...
const REGISTRATION_FIELD_ORDER=[
 "Nombre corto","Nombre del proyecto","Código del BPIP","Estado","Estado (PREVIO, REVISAR)","Etapa","¿Pertenece a algún programa?","Programa vinculado","Planes estratégicos vinculados (siglas)","Institución","Ubicación político-administrativa (provincia, cantón)","Ruta Nacional","Inversión por proyecto (Millones USD)","Coordenada Este (CRTM 05)","Coordenada Norte (CRTM 05) ","Tipo de Obra","Sub tipo de obra","Identificación de Fuentes y Modalidades de Financiamiento","Otras posibles formas de financiamiento","Comentarios adicionales sobre el financiamiento","Estado revisión","Observaciones","Completo"
];
const REG_NUMERIC_RULES={
 [hnorm("Inversión por proyecto (Millones USD)")]:{min:0.000001,max:1000000,label:"La inversión"},
 [hnorm("Coordenada Este (CRTM 05)")]:{min:0.000001,max:10000000,label:"La coordenada Este"},
 [hnorm("Coordenada Norte (CRTM 05) ")]:{min:0.000001,max:10000000,label:"La coordenada Norte"}
};
const REG_UPPERCASE_FIELDS=new Set([hnorm("Código del BPIP"),hnorm("Planes estratégicos vinculados (siglas)"),hnorm("Ruta Nacional")]);
function registrationMaxLength(h){return /coment|observ|identificaci[oó]n de fuentes|otras posibles/i.test(h)?1500:/nombre del proyecto|programa vinculado|ubicaci[oó]n/i.test(h)?500:120;}
function escapeReg(v){return String(v??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));}
function numericRegInput(el){let v=el.value.replace(/,/g,'.').replace(/[^0-9.]/g,'');const a=v.split('.');if(a.length>2)v=a.shift()+'.'+a.join('');el.value=v;validateRegistrationField(el);}
function normalizeRegistrationText(el){if(!el||!el.dataset.excelHeader)return;const h=hnorm(el.dataset.excelHeader);let v=el.value.trim().replace(/\s+/g,' ');if(REG_UPPERCASE_FIELDS.has(h))v=v.toUpperCase();else if(v)v=v.charAt(0).toUpperCase()+v.slice(1);el.value=v;validateRegistrationField(el);}
function regErrorNode(el){let n=el.parentElement?.querySelector('.field-error');if(!n&&el.parentElement){n=document.createElement('div');n.className='field-error';n.setAttribute('role','alert');el.parentElement.appendChild(n);}return n;}
function validateRegistrationField(el){if(!el?.dataset?.excelHeader)return true;const h=el.dataset.excelHeader,v=el.value.trim(),rule=REG_NUMERIC_RULES[hnorm(h)];let msg='';if(el.required&&!v)msg='Este campo es obligatorio.';if(!msg&&rule&&v){if(!/^\d+(\.\d+)?$/.test(v))msg='Digite un número válido. Ejemplo: 125.50';else{const n=Number(v);if(!Number.isFinite(n)||n<rule.min||n>rule.max)msg=`${rule.label} debe ser mayor que 0 y menor o igual a ${rule.max.toLocaleString('es-CR')}.`;}}el.classList.toggle('field-invalid',!!msg);el.setAttribute('aria-invalid',msg?'true':'false');const n=regErrorNode(el);if(n)n.textContent=msg;return !msg;}
function validateRegistrationForm(){const els=[...document.querySelectorAll('#excelProjectForm [data-excel-header]')];let ok=true,first=null;for(const el of els){if(!validateRegistrationField(el)){ok=false;if(!first)first=el;}}if(first){first.focus();first.scrollIntoView({behavior:'smooth',block:'center'});}return ok;}
function renderRegistrationForm(data={},existingGeometry=undefined){
 const box=document.getElementById("excelProjectForm");if(!box||!excelHeaders.length)return;
 const available=excelHeaders.filter(h=>!AUTO_HEADERS.some(a=>hnorm(a)===hnorm(h)));
 const fields=[...REGISTRATION_FIELD_ORDER.map(w=>available.find(h=>hnorm(h)===hnorm(w))).filter(Boolean),...available.filter(h=>!REGISTRATION_FIELD_ORDER.some(w=>hnorm(w)===hnorm(h)))];
 box.innerHTML=fields.map(h=>{const val=data[h]??"",opts=FORM_DROPDOWNS[h],required=/nombre corto|nombre del proyecto|instituci[oó]n/i.test(h),star=required?' <span class="required-star" aria-hidden="true">*</span>':'';
  if(opts)return `<div class="reg-field"><label>${escapeReg(h)}${star}</label><select data-excel-header="${escapeReg(h)}" ${required?'required':''} onchange="validateRegistrationField(this)"><option value="">Seleccione…</option>${opts.map(o=>`<option ${String(val)===o?'selected':''}>${escapeReg(o)}</option>`).join("")}</select></div>`;
  if(hnorm(h)===hnorm("Institución")){const opts=["MOPT","CONAVI","COSEVI","INCOFER","DGAC","INCOP","JAPDEVA","AYA","CTP","CNC"];return `<div class="reg-field"><label>${escapeReg(h)}${star}</label><select data-excel-header="${escapeReg(h)}" required onchange="validateRegistrationField(this)"><option value="">Seleccione…</option>${opts.map(o=>`<option ${String(val).toUpperCase()===o?'selected':''}>${o}</option>`).join("")}</select></div>`;}
  const rule=REG_NUMERIC_RULES[hnorm(h)],long=/nombre del proyecto|coment|observ|planes|identificaci[oó]n de fuentes|otras posibles/i.test(h),max=registrationMaxLength(h),attrs=`data-excel-header="${escapeReg(h)}" maxlength="${max}" ${required?'required':''}`;
  if(rule)return `<div class="reg-field"><label>${escapeReg(h)}${star}</label><input ${attrs} inputmode="decimal" value="${escapeReg(val)}" oninput="numericRegInput(this)" onblur="validateRegistrationField(this)"><small class="field-hint">Solo números · mayor que 0 · máximo ${rule.max.toLocaleString('es-CR')}</small></div>`;
  return `<div class="reg-field"><label>${escapeReg(h)}${star}</label>${long?`<textarea ${attrs} rows="3" onblur="normalizeRegistrationText(this)">${escapeReg(val)}</textarea>`:`<input ${attrs} value="${escapeReg(val)}" onblur="normalizeRegistrationText(this)">`}<small class="field-hint">Máximo ${max} caracteres${REG_UPPERCASE_FIELDS.has(hnorm(h))?' · se normaliza a MAYÚSCULAS':' · formato tipo oración'}</small></div>`;
 }).join("");
 box.insertAdjacentHTML("beforeend",`<div class="location-editor">
  ...
  <div class="location-controls">
   <div><label>Tipo de ubicación <span class="required-star">*</span></label><select id="locationType" onchange="changeLocationType()"><option value="">Seleccione…</option><option value="point">Puntual</option><option value="segment">Tramo</option></select></div>
   <div id="segmentRouteChoice" style="display:none"><label>¿Cómo desea trazar el tramo? <span class="required-star">*</span></label><select id="segmentRouteMode" onchange="changeSegmentRouteMode()"><option value="">Seleccione…</option><option value="road">Calcar rutas existentes</option><option value="linear">Línea recta entre puntos</option></select></div>
 ...
 const geometryForThisRender=existingGeometry!==undefined?existingGeometry:(window.__editingGeometry||null);setTimeout(()=>initRegistrationLocation(geometryForThisRender),120);
}
window.validateRegistrationField=validateRegistrationField;window.numericRegInput=numericRegInput;window.normalizeRegistrationText=normalizeRegistrationText;
...
async function editProjectExcel(id){
 try{
  const p=projects.find(x=>x.id===id);if(!p)return alert("No se encontró el proyecto en el HUB.");
  // El Excel maestro ya llega desde el Worker en /projects. No volver a Microsoft Graph desde el navegador.
  const data={...(p.excelData||{})};
  let g=await loadProjectGeometry(id);
  if(!g){const x=inferGeo(p);g={type:"point",source:"inferred",referencePoints:[{lat:x.lat,lon:x.lon}],inferredFrom:x.fuente};}
  window.__editingGeometry=g;
  // Mostrar primero la vista para que Leaflet reciba un contenedor con dimensiones reales.
  showView("registrar");
  renderRegistrationForm(data,g);
  const btn=document.getElementById("saveExcelProjectBtn");
  if(btn){btn.textContent="Guardar cambios";btn.dataset.editId=id;}
  // Reinicialización explícita: evita que una inicialización anterior borre la geometría cargada.
  await new Promise(r=>setTimeout(r,180));
  await initRegistrationLocation(g);
 }catch(e){console.error(e);alert("No fue posible abrir la iniciativa para edición: "+e.message);}
}
async function softDeleteProjectExcel(id){
 if(!confirm(`¿Marcar ${id} como Eliminado? El registro se conservará en Excel.`))return;
 try{
  await hubApiFetch(`/projects/${encodeURIComponent(id)}`,{method:"DELETE"});
  await loadProjectsFromExcel();
  alert(`${id} quedó marcado como Eliminado.`);
 }catch(e){console.error(e);alert("No fue posible eliminar lógicamente la iniciativa: "+e.message);}
}
...
async function loadProjectGeometry(id){
 try{
  const r=await hubApiFetch(`/projects/${encodeURIComponent(id)}/geometry`);
  return r?.geometry||r?.data||r||null;
 }catch(e){
  // Un proyecto histórico puede no tener geometría persistida todavía.
  if(/404|no encontrada|no existe/i.test(String(e?.message||e)))return null;
  console.warn("Geometría no disponible mediante HUB API",e);return null;
 }
}
...
window.HUB_ONEDRIVE_BUILD = "2026-09-21-replace-verify-v16";
console.info("HUB BUILD",window.HUB_ONEDRIVE_BUILD);
...
async function loadTraceHistory(projectId){
 ...
 box.innerHTML='<p class="muted">Cargando trazabilidad del proyecto…</p>';
 try{
  const result=await hubApiFetch(`/projects/${encodeURIComponent(projectId)}/traceability`);
  const events=Array.isArray(result?.events)?result.events:(Array.isArray(result?.traceability)?result.traceability:(Array.isArray(result)?result:[]));
  if(!events.length){box.innerHTML='<div class="note">Todavía no hay movimientos registrados para este proyecto.</div>';return;}
  box.innerHTML=`<div style="overflow:auto"><table><thead><tr><th>Fecha y hora</th><th>Usuario</th><th>Acción</th><th>Elemento</th><th>Detalles</th></tr></thead><tbody>${events.map(e=>`<tr><td>${traceEscape(new Date(e.fecha||e.date||e.createdAt||e.timestamp||Date.now()).toLocaleString())}</td><td><b>${traceEscape(e.nombreHUB||e.usuarioHUB||e.nombreMicrosoft||e.updatedBy||"Usuario")}</b><br><span class="muted">${traceEscape(e.perfil||e.role||"")}</span></td><td>${traceEscape(traceActionLabel(e.accion||e.action||e.tipo||"Movimiento"))}</td><td>${traceEscape(e.documento||e.projectId||projectId||"—")}</td><td><details><summary>Ver detalles</summary><div class="muted" style="padding-top:8px"><b>Cuenta Microsoft:</b> ${traceEscape(e.usuarioMicrosoft||e.email||"—")}<br><b>Comentario:</b> ${traceEscape(e.comentario||e.comment||e.descripcion||"—")}</div></details></td></tr>`).join("")}</tbody></table></div>`;
 }catch(e){console.error(e);box.innerHTML=`<div class="note">No fue posible cargar la trazabilidad mediante el HUB API: ${traceEscape(e.message)}</div>`;}
}
window.loadTraceHistory=loadTraceHistory;

async function syncProjectDocuments(id){
 const p=projects.find(x=>x.id===id); if(!p)throw new Error("No se encontró el proyecto "+id+".");
 const result=await hubApiFetch(`/projects/${encodeURIComponent(id)}/documents`);
 const items=Array.isArray(result?.documents)?result.documents:(Array.isArray(result?.items)?result.items:(Array.isArray(result?.value)?result.value:[]));
 const remote=items.map(x=>({
  project:id,type:"Soporte cargado",name:x.name||"Documento",state:"En OneDrive",owner:"Repositorio MOPT",
  itemId:x.id||x.itemId||"",webUrl:x.webUrl||"",downloadUrl:x.downloadUrl||"",size:Number(x.size||0),
  uploadedAt:x.lastModifiedDateTime||x.uploadedAt||"",mimeType:x.mimeType||x.file?.mimeType||""
 }));
...
async function openOneDriveDocument(d){
 const requestKey=`${d?.project||""}:${d?.itemId||""}`;
 window.__hubDocOpenInFlight=window.__hubDocOpenInFlight||new Set();
 if(window.__hubDocOpenInFlight.has(requestKey))return;
 window.__hubDocOpenInFlight.add(requestKey);
 if(!d?.project||!d?.itemId){alert("El documento no tiene un archivo físico asociado.");return;}
 try{
  showHubOperationProgress("Abriendo documento",`Descargando ${d.name||"documento"} desde el repositorio central…`,25);
  const token=await hubApiToken(true);
  ...
  const url=`${HUB_API.baseUrl}/projects/${encodeURIComponent(d.project)}/documents/${encodeURIComponent(d.itemId)}/content`;
  const res=await fetch(url,{headers});
  if(!res.ok){
   const ct=res.headers.get("content-type")||"";
   const data=ct.includes("json")?await res.json():await res.text();
   throw new Error(data?.error||data?.message||`HUB API ${res.status}`);
  }
  setHubOperationProgress(75,"Preparando el archivo para abrirlo…");
  const blob=await res.blob();
  const objectUrl=URL.createObjectURL(blob);
  const opened=window.open(objectUrl,"_blank","noopener");
  if(!opened){
   const a=document.createElement("a");
   a.href=objectUrl;a.download=d.name||"documento";document.body.appendChild(a);a.click();a.remove();
  }
  setTimeout(()=>URL.revokeObjectURL(objectUrl),120000);
  finishHubOperationProgress("Documento disponible");
 }catch(err){
  console.error(err);
  failHubOperationProgress("No fue posible abrir el documento");
  setTimeout(()=>alert("No fue posible abrir el documento: "+(err.message||err)),350);
 }finally{
  window.__hubDocOpenInFlight.delete(requestKey);
 }
}

async function editOneDriveDocument(d){
 if(!d?.project||!d?.itemId){alert("El documento no tiene un archivo físico asociado.");return;}
 try{
  // Abrir una pestaña inmediatamente para evitar que el navegador bloquee el popup
  // mientras esperamos la respuesta del Worker.
  const editWindow=window.open("about:blank","_blank");
  if(editWindow){
   try{
    editWindow.document.title="Abriendo Microsoft 365…";
    editWindow.document.body.innerHTML="<p style='font-family:Arial,sans-serif;padding:24px'>Preparando edición en Microsoft 365…</p>";
   }catch(_){}
  }

  showHubOperationProgress("Editando en línea",`Preparando ${d.name||"documento"} en Microsoft 365…`,25);

  const result=await hubApiFetch(
   `/projects/${encodeURIComponent(d.project)}/documents/${encodeURIComponent(d.itemId)}/edit-link`,
   {method:"POST"}
  );

  const editUrl=String(result?.editUrl||"").trim();
  if(!editUrl)throw new Error("El HUB no recibió el vínculo de edición de Microsoft 365.");

  setHubOperationProgress(80,"Abriendo Word/Excel en Microsoft 365…");

  if(editWindow){
   editWindow.location.replace(editUrl);
  }else{
   const opened=window.open(editUrl,"_blank","noopener");
   if(!opened)throw new Error("El navegador bloqueó la ventana de Microsoft 365. Habilite ventanas emergentes para este HUB.");
  }

  finishHubOperationProgress("Documento abierto para edición");
 }catch(err){
  console.error(err);
  failHubOperationProgress("No fue posible abrir la edición en línea");
  setTimeout(()=>alert("No fue posible editar el documento en línea: "+(err.message||err)),350);
 }
}

async function replaceOneDriveDocument(index){
 const d=documents[index],p=projects.find(x=>x.id===d?.project);
 if(!d||!p||!d.itemId){alert("Este documento no está vinculado a un archivo físico.");return;}
 ...
 input.onchange=async()=>{
  ...
  if(!confirm(`¿Sobrescribir "${d.name}" con el contenido de "${selected.name}"?\n\nEl archivo conservará su nombre e ID en OneDrive y quedará como una nueva versión.`))return;

  try{
   showHubOperationProgress("Sobrescribiendo documento",`Preparando ${d.name}…`,15);

   // Se conserva el nombre del archivo existente, pero se envían los bytes
   // del archivo seleccionado.
   const file=selected.name===d.name
    ? selected
    : new File([selected],d.name,{
       type:selected.type||"application/octet-stream",
       lastModified:selected.lastModified
      });

   const form=new FormData();
   form.append("file",file,d.name);

   setHubOperationProgress(45,"Enviando el contenido de la nueva versión…");

   const result=await hubApiFetch(
    `/projects/${encodeURIComponent(p.id)}/documents/replace`,
    {method:"PUT",body:form}
   );

   if(!result?.ok)throw new Error(result?.message||"El servidor no confirmó el reemplazo.");

   const oldTag=String(result?.versioning?.previousETag||"");
   const newTag=String(result?.versioning?.newETag||"");
   const sameItem=result?.versioning?.sameDriveItem===true;

   setHubOperationProgress(78,"Verificando la nueva versión en OneDrive…");

   // Volver a consultar OneDrive después del PUT para que la interfaz no
   // dependa del estado anterior en memoria.
   await syncProjectDocuments(p.id);

   const refreshed=documents.find(x=>
    x.project===p.id &&
    (String(x.itemId||"")===String(d.itemId||"") || String(x.name||"")===String(d.name||""))
   );

   if(!refreshed)throw new Error("OneDrive confirmó el reemplazo, pero el HUB no pudo volver a localizar el archivo.");

   finishHubOperationProgress("Documento sobrescrito y verificado");

   const versionChanged=!oldTag||!newTag||oldTag!==newTag;
   setTimeout(()=>alert(
    `Documento sobrescrito correctamente.\n\n`+
    `Archivo: ${d.name}\n`+
    `Mismo archivo en OneDrive: ${sameItem?"Sí":"Por verificar"}\n`+
    `Nueva versión detectada: ${versionChanged?"Sí":"No se pudo confirmar por ETag"}\n`+
    `Tamaño enviado: ${Math.round(selected.size/1024)} KB`
   ),250);

  }catch(err){
   console.error(err);
   failHubOperationProgress("No fue posible sobrescribir el documento");
   setTimeout(()=>alert("No fue posible sobrescribir el documento: "+(err.message||err)),350);
  }
 };
...
async function deleteOneDriveDocument(index){
 const d=documents[index],p=projects.find(x=>x.id===d?.project);
 if(!d||!p||!d.itemId){alert("Este documento no tiene un archivo físico asociado.");return;}
 if(!confirm(`¿Eliminar ${d.name} de OneDrive? La acción quedará registrada en trazabilidad.`))return;
 try{
  showHubOperationProgress("Eliminando documento",`Eliminando ${d.name}…`,25);
  await hubApiFetch(`/projects/${encodeURIComponent(p.id)}/documents/${encodeURIComponent(d.itemId)}`,{method:"DELETE"});
  await syncProjectDocuments(p.id); finishHubOperationProgress("Documento eliminado");
 }catch(err){console.error(err);failHubOperationProgress("No fue posible eliminar el documento");setTimeout(()=>alert(String(err?.message||err).includes("423")?"El documento está abierto o bloqueado en Microsoft 365. Ciérrelo y vuelva a intentarlo.":"No fue posible eliminar el documento: "+(err.message||err)),350);}
}
window.syncProjectDocuments=syncProjectDocuments;window.openOneDriveDocument=openOneDriveDocument;window.editOneDriveDocument=editOneDriveDocument;window.replaceOneDriveDocument=replaceOneDriveDocument;window.deleteOneDriveDocument=deleteOneDriveDocument;
const roleInfo={
 ...
 administrador:["Supervisa todo el portafolio","Configura plazos estándar","Decide prórrogas y solicita actualizaciones"]
};
...
function showView(id,btn){
 if(["bandeja","notificaciones","registrar","documentos","administracion"].includes(id)&&currentRole==="visitor"){
 ...
 if(id==="matriz")renderMatrix(); if(id==="historial")renderDetail(); if(id==="listado")renderList(); if(id==="bandeja")renderInbox(); if(id==="notificaciones")renderNotifications(); if(id==="documentos")renderDocuments(); if(id==="administracion")renderAdminGovernance();
 if(id==="registrar"){[80,220,500].forEach(ms=>setTimeout(()=>{try{registrationMap?.invalidateSize(true)}catch(e){}},ms));}
...
function previewRole(){
 ...
 const roleName=currentRole==="administrador"?baseRoleName:`${baseRoleName} (${ROLE_DISPLAY_STEPS[currentRole]||"—"}/${TOTAL_WORKFLOW_STEPS})`;
...
function refreshRoleAccess(){
 ...
 if(roleEl)[...roleEl.options].forEach(o=>{if(!o.value)return;if(!o.dataset.baseLabel)o.dataset.baseLabel=o.textContent.replace(/\s*\([^)]*\/17\)\s*$/,"");o.textContent=o.value==="administrador"?o.dataset.baseLabel:`${o.dataset.baseLabel} (${ROLE_DISPLAY_STEPS[o.value]||"—"}/${TOTAL_WORKFLOW_STEPS})`;});
 document.querySelectorAll(".protected-tab").forEach(t=>t.classList.remove("hidden"));
 const adminTab=document.querySelector(".admin-tab");
 if(adminTab) adminTab.classList.toggle("hidden",currentRole!=="administrador");
 const inboxTab=[...document.querySelectorAll(".protected-tab")].find(t=>t.textContent.trim().startsWith("Bandeja"));
 if(inboxTab) inboxTab.classList.toggle("hidden",currentRole==="administrador");
 ...
 previewRole(); renderInbox(); renderNotifications(); renderDocuments(); if(currentRole==="administrador")renderAdminGovernance();
}
...
async function loginRole(){
 ...
 try{
   ...
   if(loginBtn){loginBtn.disabled=true;loginBtn.textContent="Sesión iniciada";loginBtn.setAttribute("aria-disabled","true");}
   ...
   if(loginBtn){loginBtn.disabled=false;loginBtn.textContent="Iniciar sesión";loginBtn.removeAttribute("aria-disabled");}
...
function logoutRole(){
 ...
 const loginBtn=document.getElementById("loginBtn");if(loginBtn){loginBtn.disabled=false;loginBtn.textContent="Iniciar sesión";loginBtn.removeAttribute("aria-disabled");}
...
(function injectHubModalLayerFix(){
 if(document.getElementById("hub-modal-layer-fix"))return;
 const st=document.createElement("style");st.id="hub-modal-layer-fix";
 st.textContent=`
 body > .modal.open, body > [class*="modal"].open { position:fixed !important; z-index:2147483640 !important; isolation:isolate !important; transform:none !important; }
 body > .modal.open > *, body > [class*="modal"].open > * { position:relative; z-index:2147483641 !important; }
 body:has(.modal.open) .leaflet-container, body:has([class*="modal"].open) .leaflet-container { z-index:0 !important; }
 .portfolio-projects-map,.project-location-map { position:relative; z-index:0 !important; }
 `;
 document.head.appendChild(st);
})();

function elevateHubManageModal(){
 if(!manageModal)return;
 if(manageModal.parentElement!==document.body)document.body.appendChild(manageModal);
 Object.assign(manageModal.style,{
  position:"fixed",inset:"0",zIndex:"2147483640",isolation:"isolate",transform:"translateZ(0)"
 });
 manageModal.querySelectorAll("*").forEach(el=>{
  if(el.classList?.contains("leaflet-container"))el.style.zIndex="1";
 });
 const panel=manageModal.firstElementChild;
 if(panel){panel.style.position="relative";panel.style.zIndex="2147483641";}
}
      ...
      ${related.map(d=>{
        ...
        return `<div class="project-doc-item">
          ...
          <div class="project-doc-actions">
            <button onclick="openOneDriveDocument(documents[${idx}])">Descargar</button>${canEditDoc(d.type)?`<button onclick="editOneDriveDocument(documents[${idx}])">Editar en línea</button><button onclick="replaceOneDriveDocument(${idx})">Sobrescribir</button>`:""}
          </div>
        </div>`;
      }).join("")}
    </div>`;
  ...
  const table=`<div class="project-list-wrap"><table class="project-list-table">
    ...
    <tbody>${list.slice(0,400).map(p=>{
      ...
      return `<tr class="project-click-row" onclick="if(!event.target.closest('button,a,input,select,textarea,label')) focusProjectFromList('${p.id}')">
        ...
        <td class="pl-info"><span class="badge">${p.lifecycleState||p.state||"Sin estado"}</span><div class="project-sub">${phaseName(phaseOf(p.step))} · Paso ${p.step}</div><div class="project-sub"><b>${governanceBadge(p)}</b></div></td>
...
function renderInbox(){
 ...
 let list=projects.filter(p=>{if(p.lifecycleState==='Ejecutado')return false;if(currentRole==='administrador')return true;const a=p.assignment;if(a?.responsibleRole)return String(a.responsibleRole).toLowerCase().includes(String(steps.find(s=>s.n===p.step)?.role||'').toLowerCase())||canManageProject(p);return canManageProject(p);});
 const due=a=>a?.dueAt?new Date(a.dueAt).toLocaleDateString():'—';
 inboxBox.innerHTML=list.length?`${isGovernanceAdmin()?`<div class="action-row" style="margin-bottom:12px"><button class="primary" onclick="configureGovernanceDeadlines()">Configurar plazos de los 17 pasos</button></div>`:''}<div class="note" style="margin-bottom:12px"><b>${list.length}</b> proyecto${list.length===1?'':'s'} en la bandeja. La vigencia y las asignaciones se sincronizan desde el repositorio central.</div><table><thead><tr><th>Proyecto</th><th>Actividad / responsable</th><th>Vigencia</th><th>Vencimiento</th><th>Gestión</th></tr></thead><tbody>${list.slice(0,100).map(p=>{const a=p.assignment;return `<tr><td><b>${p.id}</b><br>${p.name}</td><td>${a?.activity||('Paso '+p.step+': '+(steps.find(s=>s.n==p.step)?.title||''))}<br><span class="muted">${a?.responsible||p.owner||''}</span></td><td>${governanceBadge(p)}</td><td>${due(a)}${a?.status?`<br><span class="muted">${a.status}</span>`:''}</td><td><button onclick="openManageModal('${p.id}')">Gestionar</button></td></tr>`}).join('')}</tbody></table>`:'<div class="note">No hay proyectos pendientes para este rol.</div>';
 refreshWorkBadges();
}
async function renderNotifications(){
 ...
 const base=notificationProjects(); const freshness=projects.filter(p=>deriveFreshness(p).code==='update_required'&&p.lifecycleState!=='Ejecutado'); const due=projects.filter(p=>p.assignment?.dueAt&&new Date(p.assignment.dueAt)<new Date()&&p.lifecycleState!=='Ejecutado'); const all=[...new Map([...base,...freshness,...due].map(p=>[p.id,p])).values()].sort((a,b)=>String(b.sharedUpdatedAt||b.governance?.updatedAt||'').localeCompare(String(a.sharedUpdatedAt||a.governance?.updatedAt||'')));
 const seen=getSeenNotifications();
 ...
 notifBox.innerHTML=all.length?`<div class="mail-notification-list">${all.map(p=>{
  ...
  return `<div class="note" style="border-left:5px solid ${unread?"#c62828":"#94a3b8"};padding:14px 16px;margin-bottom:10px;${unread?"background:#fff8f8":""}">
   ...
   <div class="muted" style="margin-top:5px">Asignado a: ${traceEscape(p.assignment?.responsible||s?.role||p.owner)} · Paso ${p.step}/${TOTAL_WORKFLOW_STEPS}: ${traceEscape(p.assignment?.activity||s?.title||"")}</div><div style="margin-top:5px"><b>${governanceBadge(p)}</b>${p.assignment?.dueAt?` · Vence: ${new Date(p.assignment.dueAt).toLocaleDateString()}`:""}</div>
...
function openManageModal(id,skipOneDriveSync=false){
 ...
 manageBody.innerHTML=`<div class="manage-grid">
  <div class="manage-card">
    ...
    <div class="note"><b>Vigencia:</b> ${governanceBadge(p)}<br><b>Ciclo de vida:</b> ${p.lifecycleState||p.state||"—"}${p.assignment?.dueAt?`<br><b>Plazo del paso:</b> ${p.assignment.businessDays||stepDeadline(p.step)} días hábiles · vence ${new Date(p.assignment.dueAt).toLocaleDateString()}`:""}${p.governance?.updateRequest?.status==="Pendiente"?`<br><b>Actualización solicitada:</b> vence ${new Date(p.governance.updateRequest.dueAt).toLocaleDateString()}`:""}</div><div class="action-row" style="margin:10px 0"><button type="button" class="secondary" onclick="confirmProjectUpdate('${p.id}')">Confirmar actualización</button>${isGovernanceAdmin()?`<button type="button" class="secondary" onclick="requestProjectUpdate('${p.id}')">Solicitar actualización</button>`:""}${p.assignment&&!isGovernanceAdmin()?`<button type="button" class="secondary" onclick="requestExtension('${p.id}')">Solicitar prórroga</button>`:""}${isGovernanceAdmin()&&p.assignment?.extension?.status==="Solicitada"?`<button type="button" class="btn-approve" onclick="decideExtension('${p.id}',true)">Aprobar prórroga</button><button type="button" class="btn-reject" onclick="decideExtension('${p.id}',false)">Rechazar prórroga</button>`:""}${p.step>=14&&p.lifecycleState!=="Ejecutado"?`<button type="button" class="btn-approve" onclick="markProjectExecuted('${p.id}')">Marcar como ejecutado</button>`:""}</div>
...
async function manageDecision(id,decision){
 ...
 try{
  // Persistencia centralizada: flujo + movimiento se guardan mediante el Worker.
  // No usar Graph delegado desde el navegador para la gestión.
  setHubOperationProgress(45,"Guardando trazabilidad y flujo compartido…");
  ...
  if(!p.rejected&&p.lifecycleState!=='Ejecutado'){const assignment=buildStepAssignment(p,comment||'');await saveProjectGovernance({assignment,lifecycleState:p.step>=14?'En ejecución':(p.lifecycleState||p.state)},id);p.assignment=assignment;}

  setHubOperationProgress(85,"Actualizando bandeja y notificaciones…");
 }catch(e){
  console.warn("No se pudo persistir el movimiento compartido mediante el Worker",e);
  failHubOperationProgress("No fue posible sincronizar la gestión");
  alert("La gestión no pudo sincronizarse con el HUB central: "+e.message);
  return;
 }
...
async function simulateDocUpload(id){
 ...
 input.onchange=async()=>{
  ...
  try{
   ...
   setHubOperationProgress(45,"Enviando el archivo al repositorio central…");
   await hubApiFetch(`/projects/${encodeURIComponent(id)}/documents`,{method:"POST",body:form});
   setHubOperationProgress(82,"Actualizando documentos del proyecto…");
   await syncProjectDocuments(id);
   if(manageModal.classList.contains("open")&&activeId===id)openManageModal(id,true);
   finishHubOperationProgress("Documento guardado correctamente");
  }catch(err){console.error(err);failHubOperationProgress("No fue posible cargar el documento");setTimeout(()=>alert("No fue posible guardar el archivo: "+(err.message||err)),350);}
 };input.click();
}
...
function renderDocuments(){
 ...
 documentsBox.innerHTML=`<div class="action-row" style="margin-bottom:12px"><button class="secondary" onclick="if(!projects.find(p=>p.id===activeId)){alert('Seleccione un proyecto activo del listado.');return;} syncProjectDocuments(activeId).then(()=>renderDocuments()).catch(e=>alert('No fue posible sincronizar documentos: '+e.message))">Sincronizar proyecto activo con OneDrive</button></div><table><thead><tr><th>Proyecto</th><th>Documento</th><th>Tipo</th><th>Estado</th><th>Responsable</th><th>Acciones según rol</th></tr></thead><tbody>${list.map(d=>{const idx=documents.indexOf(d);return `<tr><td><b>${d.project}</b></td><td>${d.name}</td><td>${d.type}</td><td><span class="badge">${d.state}</span></td><td>${d.owner}</td><td><div class="doc-actions"><button onclick="openOneDriveDocument(documents[${idx}])">Descargar</button>${canEditDoc(d.type)?`<button onclick="editOneDriveDocument(documents[${idx}])">Editar en línea</button><button onclick="replaceOneDriveDocument(${idx})">Sobrescribir</button><button class="btn-reject" onclick="deleteOneDriveDocument(${idx})">Eliminar</button>`:`<button class="disabled" onclick="alert('Documento en solo lectura para este rol.')">Solo lectura</button>`}</div></td></tr>`}).join("")}</tbody></table>`;
}
...
function setProjectSaveBusy(btn,busy,editing=false){
 if(!btn)return;
 if(busy){
  btn.disabled=true;
  btn.dataset.busy="1";
  btn.dataset.normalText=editing?"Guardar cambios":"Guardar iniciativa";
  btn.textContent=editing?"Guardando cambios…":"Guardando iniciativa…";
  btn.style.opacity="0.72";
  btn.style.cursor="wait";
 }else{
  btn.disabled=false;
  delete btn.dataset.busy;
  btn.textContent=btn.dataset.editId?"Guardar cambios":"Guardar iniciativa";
  btn.style.opacity="";
  btn.style.cursor="";
 }
}
...
async function addProject(){
 let saveBtn=null;
 try{
  if(!currentUser)return alert("Debe iniciar sesión para registrar o editar iniciativas.");
  ...
  if(!validateRegistrationForm())return alert("Revise los campos marcados. Hay información obligatoria o numérica que debe corregirse antes de guardar.");
  const actionLabel=editId?"guardar los cambios de esta iniciativa":"guardar esta nueva iniciativa";
  if(!confirm(`¿Confirma que desea ${actionLabel}?\n\nRevise la información antes de continuar.`))return;
  setProjectSaveBusy(btn,true,!!editId);
  showHubOperationProgress(editId?"Guardando cambios":"Guardando iniciativa",editId?"Actualizando la información en el HUB…":"Registrando la iniciativa en el HUB…",12);
  // Permite que el navegador pinte el estado "Guardando…" antes de iniciar las llamadas de red.
  await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
  ...
  if(editId){
   // La edición se hace exclusivamente a través del Worker; nunca Graph/Excel desde el navegador.
   setHubOperationProgress(35,"Actualizando el Excel maestro…");
   await hubApiFetch(`/projects/${encodeURIComponent(editId)}`,{
    method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)
   });
   if(projectGeometryDirty){
    if(!pendingProjectGeometry||!validProjectGeometry(pendingProjectGeometry))throw new Error("La nueva ubicación no es válida. Corríjala antes de guardar.");
    const geometryResult=await hubApiFetch(`/projects/${encodeURIComponent(editId)}/geometry`,{
     method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...pendingProjectGeometry,source:"user"})
    });
    const savedGeometry=geometryResult?.geometry||null;
    if(savedGeometry&&validProjectGeometry(savedGeometry))portfolioMapGeometries.set(editId,savedGeometry);
    else portfolioMapGeometries.delete(editId);
   }
   ...
   await saveProjectGovernance({lastValidatedAt:new Date().toISOString()},editId).catch(e=>console.warn("No se pudo actualizar vigencia",e));
   await loadProjectsFromExcel();
   auditLog.unshift({project:editId,action:"Editó iniciativa",role:actorLabel(),date:new Date().toLocaleString(),comment:"Actualización guardada mediante HUB API."});
   finishHubOperationProgress("Cambios guardados correctamente");
   alert(`Iniciativa ${editId} actualizada correctamente.`);
   return;
  }
  ...
  setHubOperationProgress(35,"Guardando información en el Excel maestro…");
  ...
  const geometryResult=await hubApiFetch(`/projects/${encodeURIComponent(id)}/geometry`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...pendingProjectGeometry,source:"user"})});
   const savedGeometry=geometryResult?.geometry||null;
   if(savedGeometry&&validProjectGeometry(savedGeometry))portfolioMapGeometries.set(id,savedGeometry);
  ...
  finishHubOperationProgress("Iniciativa guardada correctamente");
  alert(`Iniciativa ${id} registrada correctamente.\nExcel actualizado, ubicación guardada y carpeta gestionada en OneDrive:\n${folderName}${warning}`);
 }catch(e){
  console.error(e);
  failHubOperationProgress("No fue posible guardar la iniciativa");
  alert("No fue posible guardar la iniciativa mediante el HUB API: "+e.message);
 }finally{
  // Si el formulario sigue visible, siempre recuperar el botón aunque haya error.
  const currentBtn=document.getElementById("saveExcelProjectBtn")||saveBtn;
  setProjectSaveBusy(currentBtn,false,!!currentBtn?.dataset?.editId);
 }
}
...
/* V29: accesibilidad, validación y estados de carga */
#registrar{font-size:17px;line-height:1.5}#registrar h2{font-size:28px}#registrar .muted,#registrar .role-register-note{font-size:16px}
#excelProjectForm{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px 22px}#excelProjectForm .reg-field{min-width:0}
#excelProjectForm label{display:block;font-size:16px!important;line-height:1.35;font-weight:750;margin-bottom:7px;color:#24364d;letter-spacing:.01em}
#excelProjectForm input,#excelProjectForm select,#excelProjectForm textarea{width:100%;box-sizing:border-box;font-size:17px!important;line-height:1.4;min-height:48px;padding:10px 12px}
#excelProjectForm textarea{min-height:96px}.required-star{color:#b42318;font-size:20px;font-weight:900}.field-hint{display:block;margin-top:5px;color:#667085;font-size:13px;line-height:1.35}
#excelProjectForm .field-invalid{border:2px solid #b42318!important;outline:2px solid rgba(180,35,24,.12)}.field-error{color:#b42318;font-size:14px;font-weight:700;margin-top:5px;min-height:0}
#excelProjectForm .location-editor{grid-column:1/-1}.matrix,.matrix table,#homeListBox table,#listBox table,#adminGovernanceBox table,#documentsBox table{font-size:15px!important;line-height:1.4}.matrix th,.matrix td,#homeListBox th,#homeListBox td,#listBox th,#listBox td,#adminGovernanceBox th,#adminGovernanceBox td,#documentsBox th,#documentsBox td{padding-top:11px!important;padding-bottom:11px!important}
#loginBtn:disabled{opacity:.58;cursor:not-allowed;background:#667085!important}
#hubApiActivity{position:fixed;top:18px;right:18px;z-index:29990;background:#123052;color:#fff;border-radius:12px;padding:11px 16px;display:none;align-items:center;gap:10px;box-shadow:0 8px 24px rgba(0,0,0,.18);font-weight:750}#hubApiActivity.show{display:flex}.api-spinner{width:17px;height:17px;border:2px solid rgba(255,255,255,.4);border-top-color:#fff;border-radius:50%;animation:hubspin .75s linear infinite}
@media(max-width:800px){#excelProjectForm{grid-template-columns:1fr}#registrar{font-size:16px}}
`;document.head.appendChild(st)})();

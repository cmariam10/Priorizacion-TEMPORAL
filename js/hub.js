...
const CUSTOM_FILTER_EXCLUDE=new Set(["documents","history","geometry","_excelRowIndex","raw","returned","rejected"]);
function humanizeFilterKey(k){
 const labels={id:"ID HUB",bpip:"Código BPIP",name:"Nombre del proyecto",inst:"Institución / Dependencia",etapa:"Etapa",state:"Estado",categoria:"Categoría",subcategoria:"Subcategoría",vinculacion:"Vinculación",priority:"Prioridad",step:"Paso actual",owner:"Responsable actual"};
 return labels[k]||String(k).replace(/_/g," ").replace(/([a-z])([A-Z])/g,"$1 $2").replace(/^./,c=>c.toUpperCase());
}
function availablePortfolioFilterFields(){
 const keys=new Set();
 projects.forEach(p=>Object.keys(p||{}).forEach(k=>{
  const v=p[k];
  if(!CUSTOM_FILTER_EXCLUDE.has(k)&&!k.startsWith("__")&&(v==null||["string","number","boolean"].includes(typeof v)))keys.add(k);
 }));
 return [...keys].sort((a,b)=>humanizeFilterKey(a).localeCompare(humanizeFilterKey(b),"es"));
}
function fieldValuesAfterPreviousFilters(field,index){
 let list=projects.slice();
 for(let i=0;i<index;i++){const f=customPortfolioFilters[i];if(f?.field&&f.value!=="")list=list.filter(p=>String(p[f.field]??"")===String(f.value));}
 return [...new Set(list.map(p=>String(p[field]??"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"es",{numeric:true}));
}
function renderCustomFilterBuilder(){
 const box=document.getElementById("customPortfolioFilterBuilder");if(!box)return;
 const fields=availablePortfolioFilterFields(), active=customPortfolioFilters.filter(f=>f.field&&f.value!=="").length;
 box.innerHTML=`<div class="custom-filter-head"><div><b>Constructor de filtros</b><div class="muted">Elija los atributos de la tabla que desea usar. Las opciones de cada filtro se recalculan con los filtros anteriores.</div></div><span>${active} filtro${active===1?"":"s"} activo${active===1?"":"s"}</span></div>
 <div>${customPortfolioFilters.map((f,i)=>{const vals=f.field?fieldValuesAfterPreviousFilters(f.field,i):[];return `<div class="custom-filter-row">
  <select onchange="setCustomFilterField(${i},this.value)"><option value="">Escriba o seleccione una variable…</option>${fields.map(k=>`<option value="${traceEscape(k)}" ${f.field===k?"selected":""}>${traceEscape(humanizeFilterKey(k))}</option>`).join("")}</select>
  <select ${f.field?"":"disabled"} onchange="setCustomFilterValue(${i},this.value)"><option value="">Todas</option>${vals.map(v=>`<option value="${traceEscape(v)}" ${String(f.value)===v?"selected":""}>${traceEscape(v)}</option>`).join("")}</select>
  <button type="button" onclick="removeCustomPortfolioFilter(${i})">Quitar</button></div>`}).join("")}</div>
...
function addCustomPortfolioFilter(){customPortfolioFilters.push({field:"",value:""});renderCustomFilterBuilder();}
function removeCustomPortfolioFilter(i){customPortfolioFilters.splice(i,1);rerenderCustomFilters();}
function setCustomFilterField(i,field){customPortfolioFilters[i]={field,value:""};rerenderCustomFilters();}
function setCustomFilterValue(i,value){if(customPortfolioFilters[i])customPortfolioFilters[i].value=value;rerenderCustomFilters();}
function clearCustomPortfolioFilters(){customPortfolioFilters=[];rerenderCustomFilters();}
window.addCustomPortfolioFilter=addCustomPortfolioFilter;window.removeCustomPortfolioFilter=removeCustomPortfolioFilter;window.setCustomFilterField=setCustomFilterField;window.setCustomFilterValue=setCustomFilterValue;window.clearCustomPortfolioFilters=clearCustomPortfolioFilters;
...
function getGlobalFilteredProjects(){
 ...
 customPortfolioFilters.forEach(f=>{if(f?.field&&f.value!=="")result=result.filter(p=>String(p[f.field]??"")===String(f.value));});

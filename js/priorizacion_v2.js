...
function runUiAction(fn) {
  return async (...args) => {
    try {
      ...
      console.error(e);
      toast(e.message || "Ocurrió un error.");
    }
  };
}
...
document.getElementById("btnRefresh").onclick = runUiAction(loadAll);
document.getElementById("btnBatch").onclick = runUiAction(recalcBatch);
document.getElementById("btnCsv").onclick = exportCsv;
document.getElementById("btnSave").onclick = runUiAction(saveInput);
document.getElementById("btnCalculate").onclick = runUiAction(calculateActive);
document.getElementById("matrixOverride").onchange = () =>
  renderEditor({
    matrixOverride: document.getElementById("matrixOverride").value,
    inputs: { raw: {}, scores: {} }
  });
...
loadAll().catch(e => {
  console.error(e);
  toast(e.message);
  document.getElementById("methodologyNotice").textContent =
    "No fue posible cargar el módulo: " + e.message;
});

 ...
 for(const mode of ["geospatial","official","manual","managerial"]){
   ...
   for(const id of list){
     ...
     if(mode==="geospatial"||mode==="official"){
       html.push(`<div class="indicator indicator-auto" data-indicator="${id}" data-mode="${mode}"><div class="indicator-head"><b>${id}</b><span>Calculado: ${fmt(value)}</span></div><div>${esc(INDICATOR_NAMES[id]||id)}</div><div class="auto-value"><strong>${fmt(value)}</strong><span>${mode==="geospatial"?"Resultado automático del análisis espacial":"Dato automático / institucional"}</span></div>${mode==="geospatial"&&GEO_REASON[id]?`<small><b>¿Por qué geoespacial?</b> ${esc(GEO_REASON[id])}</small>`:`<small>${value===null?"Pendiente de fuente o de una corrida válida.":"Este valor no se edita manualmente."}</small>`}</div>`);
     }else{
...
const EVIDENCE_REASON={
  protectedAreas:"Permite identificar si el trazado intersecta o se aproxima a áreas protegidas; alimenta la sensibilidad ambiental.",
  asp:"Permite identificar si el trazado intersecta o se aproxima a áreas protegidas; alimenta la sensibilidad ambiental.",
  wetlands:"Los humedales son áreas ambientalmente sensibles y su proximidad/intersección afecta la valoración ambiental.",
  corridors:"Los corredores biológicos permiten reconocer continuidad ecológica potencialmente afectada por el proyecto.",
  landslides:"Los deslizamientos evidencian inestabilidad del terreno y exposición física del proyecto.",
  crowns:"Las coronas de deslizamiento complementan el análisis de inestabilidad y cercanía a movimientos de masa.",
  floods:"Las áreas con potencial de inundación permiten medir exposición a amenaza por inundación y multiamenaza.",
  roadHierarchy:"La jerarquía vial de CONAVI permite estimar la criticidad de conectividad de la vía sobre la que se ubica el proyecto."
};
...
function collectInput(){
 ...
 return{matrixOverride:document.getElementById("matrixOverride").value,raw,scores,replaceManualInputs:true,finalAggregation:{technicalWeight:tw===""?null:Number(tw),managerialWeight:gw===""?null:Number(gw)}};
}

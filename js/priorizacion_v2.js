...
const INDICATOR_ACQUISITION = {
  "ND-01":"official", "ND-02":"official", "ND-03":"official",
  "ND-04":"manual", "ND-05":"manual", "ND-06":"geospatial", "ND-07":"geospatial", "ND-08":"geospatial", "ND-09":"manual",
  "IS-01":"official", "IS-02":"official", "IS-03":"geospatial",
  "VI-01":"official", "VI-02":"manual", "VI-03":"geospatial",
  "RR-01":"geospatial", "RR-02":"official", "RR-03":"official", "RR-04":"official", "RR-05":"manual", "RR-06":"geospatial",
  "SI-01":"geospatial", "SI-02":"official", "SI-03":"official",
  "PG-01":"managerial", "PG-02":"managerial", "PG-03":"managerial", "PG-04":"managerial", "PG-05":"managerial"
};

const ACQUISITION_LABELS = {
  geospatial: {title:"Automáticos geoespaciales", note:"Se obtienen mediante cruces con capas oficiales en línea. No son editables manualmente."},
...
function evidenceLayerLabel(k){return ({asp:"Áreas silvestres protegidas",wetlands:"Humedales",corridors:"Corredores biológicos",landslides:"Deslizamientos",crowns:"Coronas de deslizamiento",floods:"Inundaciones"})[k]||k;}
...
function renderEvidence(spatial){
 ...
 for(const [k,v] of Object.entries(ev)){
  ...
  cards.push(`<div class="evidence-card"><b>${esc(k)}</b><p>Estado: ${esc(estado)}${v.typeName?`<br>Capa: ${esc(v.typeName)}`:""}${m?`<br>Intersecciones: ${m.intersectCount}<br>En ${m.nearMeters} m: ${m.nearCount}<br>Consultados: ${m.totalFetched}`:""}${v.lastValidAt?`<br>Último dato válido: ${esc(new Date(v.lastValidAt).toLocaleString())}`:""}${v.refreshError?`<br>Actualización fallida: ${esc(v.refreshError)}`:""}${v.error&&!stale?`<br>Error: ${esc(v.error)}`:""}</p></div>`);
 }
 ...
 if(summary)cards.unshift(`<div class="evidence-card"><b>Estado de fuentes</b><p>Actualizadas: ${summary.fresh??0}<br>Con dato guardado: ${summary.stale??0}<br>No disponibles: ${summary.unavailable??0}</p></div>`);
...
async function openProject(id){
 ...
 let statusHtml=d.scores?.missing?.length?`Faltan <b>${d.scores.missing.length}</b> indicadores para completar el cálculo: ${d.scores.missing.join(", ")}.`:(lastValid?`Último cálculo válido: <b>${lastValid}</b>.`:"Este proyecto todavía no ha sido calculado.");
